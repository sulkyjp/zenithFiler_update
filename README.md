...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.46.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.46.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.46.2_delta_from_0.46.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.46.2] - 2026-04-14 : 右ドラッグのOS標準ドロップメニュー対応 + Stickman AI 生成改善 + プロパティ修正

### Added
- **右ボタンドラッグでアプリ外にドロップした際、Windows 標準のドロップメニューを表示:** ファイル一覧の項目を右ボタンでドラッグしてデスクトップやエクスプローラ等にドロップすると、OS 標準の「ここにコピー／移動／ショートカットを作成／キャンセル」メニューが表示され、ショートカット（.lnk）をアプリ外に直接作成できるようになった

### Changed
- **AI 行動パターン生成ダイアログ: 結果パネルのパターン名を編集可能に:** 生成後の結果画面で名前をその場で編集でき、以降の再生成・保存に反映される

### Fixed
- **プロパティパネルのパス表示欄: 長いパスで下枠線が欠ける不具合を修正:** 外枠を固定高さにし、内容が超える場合はスリムな縦スクロールバーで参照できるようにした
- **AI 行動パターン生成ダイアログ: 再生成で確定済みのパターン名が失われる不具合を修正:** 再生成後もユーザが入力／編集した名前を踏襲するように修正
- **AI 生成／編集した Stickman 行動パターンが保存直後のプレビューで再生されない不具合を修正:** プレビューコントロールとステータスバーが保持する行動辞書キャッシュが更新されず、アプリ再起動まで新規定義が反映されなかった。保存完了時に `MascotEventBridge.StoreBehaviorInstalled` を発火させてキャッシュを即時リセットするよう修正
- **AI 生成した Stickman 行動パターンの MAKE バッジが付かず DL 表示になる不具合を修正:** リファイン時に AI 応答で DTO を置換する際、`origin`/`author` が失われ保存 JSON から `"origin": "user"` が欠落していた。リファイン後も自作フラグと作者名を確実に保持するよう修正
