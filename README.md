...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.30.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.30_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.30_delta_from_0.40.29.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.30] - 2026-04-02 : 起動高速化・演出統一・実績履歴・セキュリティ強化

### Added
- **実績履歴ポップアップ:** チャレンジページから達成済み実績の履歴を閲覧できるポップアップウィンドウを追加。カテゴリ別フィルター選択時はタイムラインフィード表示、「すべて」選択時はテーブル一覧表示。達成日の自動記録に対応
- **起動時バー成長アニメーション:** お気に入りのアクセス回数バー、最近開いたファイル/フォルダの回数バー、ファイルサイズバーが起動時に0から目標幅まで伸びるアニメーションを追加。ウェルカムメッセージ退場と同期して開始。演出設定「A. 起動・全体」でオン/オフ可能

### Changed
- **起動演出のタイミング統一:** ウェルカムアニメーション → テーマトースト → グリーティングの順序を明確に制御。ウェルカム完了後に後続の演出が開始するよう統一し、複数の表示が同時に動作するドタバタ感を解消
- **認証フロー簡素化:** キーのみで認証可能に変更。従来のメールアドレス併用方式も引き続きサポート
- **内部セキュリティ強化:** 無効化キーの検出メカニズムを追加。起動時および登録時に照合を実施
- **リネームダイアログのパターンボタンをカーソル位置挿入に変更:** 日付・親フォルダ名・カスタム定型のパターンボタンを押下した際、テキスト全体を置き換えるのではなく、入力カーソル位置にテキストを挿入するよう変更。選択範囲がある場合はその部分を置き換え

### Fixed
- **起動時のウェルカムメッセージが表示されない不具合を修正:** エフェクト設定オフ時にウェルカムメッセージが表示されていなかった問題を修正。エフェクトオフでも通知としてウェルカムメッセージを表示するよう改善。メッセージのローカライズにも対応
- **AIリクエストの max_tokens がモデル上限を超えてエラーになる不具合を修正:** モデルごとの最大出力トークン数を定義し、リクエスト送信前に自動キャップする仕組みを追加。gpt-4.1-mini 等で大量出力を要求する機能（お気に入り整理・フォルダ整理）が HTTP 400 エラーになる問題を解消
- **タブ管理ビューでタブを一番下にドラッグ移動できない不具合を修正:** 同一ペイン内の末尾位置への移動が境界チェックで誤って拒否されていた問題を修正
- **APIキー保存直後のテストで「APIキーを入力してください」と表示される不具合を修正:** デバウンス保存完了前でもインメモリキャッシュからキーを返すようにし、保存直後のテストが正常に動作するよう改善
- **「よく使うフォルダ」のスライダーを上げても一定以上表示されない不具合を修正:** 履歴DBの取得件数が少なくお気に入りとの突合後に候補が不足していたため、取得上限を50→200に拡大
- **クイックジャンプダイアログでIME変換候補が点滅する不具合を修正:** 検索デバウンスタイマーが50msと短すぎたため、IME変換中にUI再描画が頻発していた問題を300msに延長して改善
- **起動時にユーザー操作ができない時間が発生する不具合を修正:** タブ復元時のパス存在チェックがUIスレッドを同期ブロックし、ウィンドウ描画が遅延していた問題を修正。パス検証の非同期化・ファイル監視の遅延起動により起動直後から操作可能に
