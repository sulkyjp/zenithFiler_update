...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.5.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.5.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.5.0_delta_from_1.4.4.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.5.0] - 2026-07-06 : マクロスタジオの配線・操作性強化、AI設定/テーマ機構の内部リファクタリング

### Added
- **マクロスタジオの連結線がカードを避けて配置されるように変更:** ノード間の連結線が直線でカード本体と重なっていた問題を解消し、カードの矩形を避けた直交ルート（コの字型）で自動配線するようにした。カードの移動・リサイズに動的に追従する
- **マクロスタジオのキャンバスで空白を左ドラッグすると画面移動できるように変更:** 従来は右/中クリックドラッグのみだったパン操作に左ドラッグを追加。範囲選択はShift+左ドラッグへ移動した（キャンバス左下にヒントを表示）
- **マクロスタジオでカードを選択すると「次に置くと自然な部品」を左パレットにハイライト表示するように変更:** コマンド種別ごとの定番の組み合わせ（静的ルール）と、実際に繋いだ履歴（使うほど賢くなる）を組み合わせて上位3件を★おすすめ表示し、自動的にスクロールして見えるようにした
- **マクロスタジオでカードを拡大・移動すると後続カードが自動で退避するように変更:** 幅・高さの拡大や、カードのドラッグ移動で後続カード（接続で辿れる下流）と重なる場合、アニメーション付きで押し出して再配置する。ドラッグ中のカードは対象外

### Changed
- **マクロスタジオの生成コードプレビューがライブ更新されるように変更:** これまでノードの引数編集や配置変更は「⚙反映」ボタンを押すまで右ペインに反映されなかったが、編集内容が300ms後に自動でプレビューへ反映されるようにした（反映ボタンは直列ビルダーへの反映用として存置）
- **マクロスタジオの部品パレットのスクロールバーをアプリ標準のスリム表示に変更:** 既定の太いスクロールバーがタイルに密着して見づらかった問題を解消
- **マクロスタジオがテーマ切替に即座に追従するように変更:** これまでは窓を開いた時点の色で固定されていたため、テーマを変更してもノード・配線・コネクタの色が更新されなかった
- **AI設定関連コードを別ファイルに分割（動作に変更なし）:** 8つの設定ドメインが同居していた `AppSettingsViewModel.cs`（7,700行超）から、AI連携設定（Ollama管理・プロバイダ切替・接続/モデル検証・自動機能トグル等）を `AppSettingsViewModel.Ai.cs` へ partial クラスとして分離した。ロジックは一切変更していない機械的な移動のみ。今後の保守性向上に向けたリファクタリングの一環
- **AIサービス層（プロバイダ通信部分）の内部リファクタリングを実施（動作は不変）:** Claude/OpenAI/Gemini/Azure OpenAI/Ollama の各プロバイダで重複していた送信処理・エラーメッセージ抽出処理を共通化し、プロバイダ切替のディスパッチをインターフェース経由に統一した。AI設定変更のたびに発生していた設定ファイルの同期読込を排除し、接続テスト・モデル検証・モデル一覧取得に応答なし時のタイムアウト（既定30秒、Ollamaは120秒）を追加。3箇所に分かれていたエラーメッセージの変換ロジックを一元化し、APIキーのマスク表示を固定長に統一した。マクロのAI連携処理（`zen.ai()`）も同様の方針で内部実装を整理
- **テーマ管理サービス（ThemeService）の内部リファクタリングを実施（起動性能は同等・動作は不変）:** 153件の色キー定義（デフォルト値・対応ブラシ・継承関係）がこれまで3箇所のディクショナリリテラルに分散していたのを `ThemeKeyRegistry` という単一の情報源に統合した。テーマ適用時のプロパティ走査（リフレクション）を1パスに統合してマージ処理あたりの走査回数を半減させたほか、色→ブラシ変換に凍結ブラシの再利用プールを導入して同色ブラシの再生成を抑制し、「カスタム上書きの有無」判定をキャッシュ化してディスクアクセス回数を削減した。レジストリと内部モデルの整合性を検証する自動テストも追加。起動タイミング（ThemeLoadAndApply 約13ms 等）に劣化がないことを確認済み

### Fixed
- **APIキー保存直後に別のAI設定を変更すると、保存したはずのAPIキーが失われることがある競合を解消:** デバウンス保存が完了する前に他の設定変更が発生すると、ディスク上の古いキーで新しいキーが上書きされてしまうことがあった不具合を構造的に解消した。あわせて、APIキーの削除操作が保存完了まで反映されない（削除直後もAI機能が旧キーを使い続けてしまう）問題も修正した
- **standard/CloudGray/Daylight の3テーマで、ダイアログボタンやチャレンジカードなど一部の色設定（計11キー）がJSON内のセクション配置ミスにより無視され、意図した色ではなくデフォルト色にフォールバックしていた不具合を修正:** 各キーを対応する正しいセクション（Misc）に移動。色の値そのものは変更していない
