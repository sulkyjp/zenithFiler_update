...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.39.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.39.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.39.2_delta_from_0.39.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.39.2] - 2026-03-25 : Pro制限整備 + リファクタリング + UX改善

### Added
- **Pro制限追加:** ストアアップロード(1回)、レイアウトプリセット保存(Pro限定)、AIリネーム(2回)、AIバックアップ要約(2回)にFree版制限を追加。PRO機能にロックアイコンバッジを表示
- **拡張子編集:** ファイル名変更ダイアログで拡張子を分離表示し、別テキストボックスで編集可能に

### Changed
- **AIフォルダオーガナイザーの提案品質向上:** AIに送信するコンテキストを大幅強化。親フォルダ階層・兄弟フォルダ名・プロジェクト種別検出・サブフォルダのアイテム数を追加
- **初期設定変更:** デフォルトを2ペインモード+Espressoテーマ+右ナビ表示に変更
- **フルアップデートボタン廃止:** リポジトリリンクのテキスト案内に変更
- **1画面/2画面ボタン:** 選択中のモードに背景色を追加
- **お気に入りフォントサイズ:** 選択範囲を10〜20に拡張
- **バックアップ頻度最適化:** 監視対象フィールドに変更がない場合はバックアップをスキップ

### Fixed
- **ナビペイン レイアウト切替時のクラッシュ修正:** ヘッダーアクション要素の親切り離し漏れを解消
- **起動性能改善:** Task.Resultブロッキング修正、OnPropertyChangedバッチ化、async void修正
- **ビルド警告ゼロ化:** MVVMTK0039/0034/CS4014 警告を全て解消
- **AI MaxTokens不足修正:** テーマ診断(4096)、フォルダ名提案(1024)のトークン上限を増加
