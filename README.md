...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.17.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.17_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.17_delta_from_0.40.16.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.17] - 2026-03-29 : AI ローカライズ + チャレンジ修正 + 設定安定化

### Added
- **AI 設定ページに機能一覧カードを追加:** 12の AI 機能を2カラムグリッドで表示し、API キーが必要な旨を注記。10言語対応

### Changed
- **チャレンジカウンターの難読化:** settings.json のチャレンジカウンターを HMAC 署名付き Base64 エンコードに変更。旧フォーマットからの自動マイグレーション対応

### Fixed
- **AI 二つ名生成が無限リトライする問題を修正:** チャレンジ達成時に二つ名が未生成の場合、操作のたびに AI 呼び出しが繰り返されトークンを大量消費していた。生成中フラグを追加しリトライを防止
- **AI 二つ名・バックストーリー生成のローカライズ対応:** プロンプトに出力言語指定を追加し、ユーザーのロケールに応じた言語で生成。機械的フォールバックのパターン文字列もローカライズ
- **統計画面の AI 機能名がローカライズされていない問題を修正:** 「AI Challenge Title」「AI Challenge Backstory」をローカライズ対応に変更
- **AI フォルダ整理・お気に入り整理のプロンプトをローカライズ対応:** フォルダ分析のセクション見出しをロケールに応じた言語で出力。お気に入り整理の例文を英語に統一し、出力テキストをロケール言語に変更
- **初期状態でレベルバナーが表示されない問題を修正:** カウンターが空の場合にXP/レベル計算がスキップされていた。初期状態でも Lv.1 を表示するよう修正
- **チャレンジページのカテゴリ名・アンカーリンクをローカライズ:** 15カテゴリ名が日本語ハードコードだったのを全10言語対応に変更
- **AI モデル設定が終了時に失われる場合がある問題を修正:** デバウンス保存（800ms 待ち）が完了する前にウィンドウを閉じると、古い settings.json が読み込まれ設定が巻き戻っていた。終了時にデバウンス保存を即座にフラッシュするよう修正
- **AI プロバイダ切替時にモデルがリセットされる問題を修正:** プロバイダを切り替えるたびにモデルがデフォルトに戻っていた。プロバイダ別の選択モデルをメモリキャッシュし、切り替え時に前回の選択を復元するよう修正
- **二つ名・バックストーリーが自動生成されない問題を修正:** チャレンジページ外での実績解除時に二つ名・バックストーリーが更新されなかった。どのページにいても称号変化時にバックグラウンドで自動生成するよう修正
- **未使用フィールド警告を修正:** MainWindow._savedPlacement, PerfDiagnosticsService._diskMax を削除
