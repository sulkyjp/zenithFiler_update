...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.42.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.42.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.42.1_delta_from_0.42.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.42.1] - 2026-04-08 : フィードバック実績・チャレンジアイコン追加

### Added
- **フィードバック実績カテゴリ:** チャレンジに「フィードバック」区分を新設（15区分 × 10 Tier = 150種）。フィードバック送信回数に応じて「最初の声」から「このアプリの共創者」まで10段階の実績を達成可能
- **チャレンジカテゴリアイコン:** チャレンジページのカテゴリヘッダー・アンカーボタン・実績履歴ウィンドウにカテゴリ別アイコンを追加
