...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.3.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.3.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.3.0_delta_from_1.2.3.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.3.0] - 2026-06-17 : マクロ機能を追加（JavaScript で操作を自動化・ナビペイン/ステータスバー/トリガー対応）

### Added
- **マクロ機能を追加（操作を JavaScript で自動化）:** ナビペインに「マクロ」ビューを新設し、ステータスバーからも素早く実行・記録できるようにした。コードを書かなくても使えるよう GUI ビルダーと操作記録を備える
  - **zen.* API:** フォルダ移動・検索・フォルダ作成・移動・リネーム・コピー・通知・ログなどを JavaScript から呼び出せる。フォルダ操作は現在フォルダ内に限定し、実行前に安全性を検証。1 回の取り消し（Ctrl+Z）でマクロ全体を巻き戻せる
  - **GUI ビルダー:** コマンドを選んでステップを並べるだけで JavaScript を自動生成（コード不要）
  - **操作の記録:** フォルダ移動・検索などの操作を記録して JavaScript に変換し、そのまま再生・編集できる
  - **AI 連携:** 自然言語の指示から「ファイルの分類」や「整理アクションの提案」を AI に依頼し、結果に基づいて操作を実行できる（生成 AI の設定が必要）
  - **フォルダ監視（自動実行）:** 指定したフォルダに変化があったときにマクロを自動実行できる
  - **保存・呼び出し:** 作成したマクロに名前を付けて保存し、ナビペインの一覧やステータスバーのプルダウンからワンクリックで実行できる
  - **起動時の自動トリガー:** アプリ起動完了後に、保存済みマクロのフォルダ監視を自動で再開し、「起動時に実行」を有効にしたマクロを実行する。スケジュール実行を設定したマクロがあれば定期チェックを開始する（起動の速度には影響しないよう、描画完了後のアイドル時に初期化）
