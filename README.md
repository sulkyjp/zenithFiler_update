...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.2.3.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.2.3_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.2.3_delta_from_1.2.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.2.3] - 2026-06-12 : 画面端マグネットスナップとクイックジャンプの精度・視認性向上

### Added
- **クイックジャンプに「フォルダのみ / ファイルも含む」トグルを追加:** 検索対象をフォルダのみに絞り込めるようになった（既定はフォルダのみ）。状態は記憶され、次回以降も引き継がれる
- **ウィンドウの画面端マグネットスナップ:** ウィンドウをドラッグして画面端（タスクバーを除いたワークエリア）に近づけると、磁石のようにピタッと吸着するようにした（15px 以内・四隅対応・マルチモニタ＆高 DPI 対応）。そのままドラッグを続ければ引き剥がせる。Control Deck → 基本設定 → 操作 でオン/オフを切り替えられる（既定: オン）

### Changed
- **クイックジャンプの候補ソースを一目で判別できるように改善:** 各候補のバッジにソース別のカラーとアイコン（お気に入り＝アンバー★、よく使う＝グリーン、履歴＝ブルー、よく開くタブ＝パープル、最近のファイル＝スレート、インデックス＝ティール）を表示するようにした。テーマの明暗に応じて視認しやすい配色が自動選択される
- **クイックジャンプの検索精度を向上:** 完全一致 > 前方一致 > 単語境界一致 > 部分一致の段階評価、空白区切りの複数語 AND 検索、短い名前の優遇を導入し、探しているフォルダが上位に来やすくなった。カスタム表示名を付けたお気に入りも実フォルダ名で検索できるようになった
- **ナビペインのビュー選択メニューから「Box API」を非表示:** API を登録しても現状は実機能が提供されていないため、ペインメニューの選択肢から除外した（既存の配置に含まれていた場合も表示されなくなる。内部機能・登録済みデータは保持）

### Fixed
- **お気に入りがクイックジャンプの検索結果から漏れることがある問題を修正:** インデックス検索の高得点候補が多い場合に、マッチしたお気に入りが表示上限（50件）から押し出されることがあった。マッチしたお気に入りは必ず結果に含まれるようにした
