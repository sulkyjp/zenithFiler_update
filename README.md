...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.8.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.8_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.8_delta_from_0.40.7.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.8] - 2026-03-27 : チャレンジシステム超進化 — スキルツリー + 二つ名 + バックストーリー

### Added
- **100チャレンジのスキルツリー:** 10区分×10ノード、WrapPanel で全カード表示。各ノードにあおり文句・プログレスバー・ToolTip件数表示。最終ノードに王冠ゴールマーク
- **AI二つ名自動生成:** 称号変化時にAIが中二病全開の二つ名を自動生成。AI未使用時は機械的生成にフォールバック
- **AIバックストーリー生成:** 全達成実績をもとにファンタジーRPG風の英雄譚を自動生成・保存
- **称号授与証パネル:** メダルアイコン + 二つ名 + 称号バッジ + プログレスバーを統合したモダンなカードUI

### Changed
- **チャレンジ表示:** 縦リスト → スキルツリー式WrapPanel。固定幅カード + あおり文句2行 + 横バー
- **設定メニュー整理:** 基本設定 / テーマ・ストア / 機能 / データ・ライセンス / その他の5区分にセパレーター整理
- **お気に入り設定:** フォントサイズ・ピン件数の ComboBox → スライダー化

### Fixed
- **チャレンジ実績が保存されない問題:** MainWindow_Closing の設定引き継ぎにチャレンジ関連フィールドを追加
- **カウンターが毎回リセットされる問題:** WindowSettings.Load() の遅延問題をインメモリキャッシュで解決
