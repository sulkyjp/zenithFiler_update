...
### Changed
- **テーマ切替の高速化:** リソース更新を BeginInit/EndInit でバッチ化し WPF レイアウト再計算を抑制。トランジションアニメーションを 130ms→50ms / 250ms→120ms に短縮。テーマ JSON のプリロードを切替時に自動実行
...
- **自動アップデートが GitHub rate limit で失敗する問題を修正 (#86):** ダウンロード用 HttpClient を API チェック用と分離し、GitHub API ヘッダーを除去。rate limit ヘッダー（X-RateLimit-Remaining）を監視し残り少ない場合は次回チェックを6時間後に延長。ダウンロード失敗時はブラウザでリリースページを自動で開くフォールバックを追加
