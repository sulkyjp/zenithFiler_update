...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.2.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.2.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.2.2_delta_from_1.2.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.2.2] - 2026-06-12 : フォルダ新規作成時に名前入力ウィンドウが表示されない問題を修正

### Fixed
- **フォルダ新規作成時に名前入力ウィンドウが表示されないことがある問題を修正 (#169):** 名前入力ウィンドウは前回表示した位置を記憶するため、外部モニタを切断した後（ドック解除など）に切断済みモニタ上＝画面外へ表示され、「ウィンドウが開かない」ように見えることがあった。表示のたびにメインウィンドウ中心へ再配置し、必ず画面内に収まるよう補正した（リネーム・各種入力ウィンドウ共通）。また、クラウド同期フォルダ等で作成直後のフォルダが一覧に現れるまで時間がかかる場合に名前入力が始まらないまま無反応になる経路を改善し、待機しても開始できなかった場合は「F2 キーで変更できます」と必ず通知するようにした
