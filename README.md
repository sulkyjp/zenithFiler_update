...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.0.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.0.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.0.1_delta_from_1.0.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.0.1] - 2026-06-02 : 操作の手応え強化（行ハイライト・完了トースト）＋ペイン/アプリ表示トグルのショートカット＋検索バー Esc 一発リセット

### Added
- **左右ナビペインの表示/非表示ショートカット + アプリ表示/非表示のグローバルトグル:** 左ナビペイン・右ナビペインそれぞれを開閉するキーボードショートカットを追加した（既定: 左 = `Ctrl+Shift+[`、右 = `Ctrl+Shift+]`）。設定 → ショートカットから任意のキーに変更できる。あわせて、グローバルホットキー（既定 `Ctrl+Shift+E`）をこれまでの「前面化のみ」から**表示/非表示トグル**に変更した。最前面に表示中に押すとアプリを隠し（常駐モードONならトレイへ格納してタスクバーから消す／OFFなら最小化）、隠れている・最小化・背面のときは前面化する。隠したあとはホットキーまたはトレイアイコンで復帰できる
- **削除・コピー/移動の「手応え」を強化:** ファイルを削除すると次に選ばれた行を一瞬ハイライトして視線を誘導するようにした（演出設定「リスト演出」連動）。コピー/移動の完了時には「N 件コピー/移動しました」をトーストで通知。ディレクトリツリーでも現在地のフォルダが選択されると一瞬ハイライトして「今ここ」が分かるようにした
- **インクリメンタルサーチの入力を可視化:** 一覧でキーをタイプして頭出し検索する際、入力中の文字列をステータスバーに「検索: ○○」と一時表示するようにした

### Changed
- **検索バーの Esc を一発フルリセットにするオプションを追加:** 設定 → 検索に「Esc を一発で全リセット」を追加した。オンにすると検索バーで Esc を一度押すだけでテキスト・サイズ/日付フィルタ・スコープをまとめてクリアする（オフ時は従来どおり、1回目でテキスト・2回目でフィルタの2段階）
- **オプション画面のマウスホイール移動量を改善:** Control Deck（設定画面）の各カテゴリは縦長な設定リストを素のスクロール領域で表示していたため、マウスホイール1ノッチあたりの移動量が小さく「もっさり」と感じられた。1ノッチあたりのスクロール量を引き上げ、ファイル一覧に近い軽快な操作感にした。テーマストアやチャレンジなど内側に独立したスクロール領域がある場合は、そちらの従来挙動をそのまま維持する

### Fixed
- **作成した起動時プリセットが再起動するまで一覧に出ない不具合を修正:** 設定 → 基本設定の「起動時プリセット」のドロップダウン（ウィンドウ配置／ナビペイン／ワーキングセット）は、選択肢を起動時に一度だけ構築していたため、作成・リネーム・削除したプリセットがアプリを再起動するまで反映されなかった。Control Deck（設定画面）を開くたびに、未保存分を確定してから最新の settings.json で選択肢を再構築するようにし、作成直後でも即座に選べるようにした
