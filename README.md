...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.39.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.39.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.39.1_delta_from_0.39.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.39.1] - 2026-03-25 : レイアウトプリセット + テーマストア診断 + バグ修正

### Added
- **ナビペイン レイアウトプリセット:** ナビペインの配置（左上/左下/右上/右下のビュー配置、お気に入り内容、ゾーン設定）をプリセットとして保存・復元。ビルトイン4種（標準・コンパクト・デュアル・作者構成）＋ユーザー保存。ステータスバーのプリセットボタンからワンクリック切替
- **テーマストア版テーマ診断:** ストアのテーマカタログを対象としたテーマ診断（非AI版・AI版）を追加。ストアツールバーのSparkles/BrainCircuitアイコンから起動。推薦結果からプレビュー＆ダウンロードが可能

### Fixed

- **ナビペイン レイアウト切替時のクラッシュ修正:** プリセット適用・ドック切替・ゾーン変更時に「指定された要素は、既に別の要素の論理子です」エラーでクラッシュする問題を修正。ビジュアルツリーとロジカルツリーの不一致によるビュー切り離し漏れ、およびヘッダーアクション要素の親切り離し漏れを解消
- **ターミナル初回起動クラッシュの修正:** WebView2初期化完了前にテーマ適用イベントが発火し、未初期化のWebView2にアクセスしてアプリが異常終了する問題を修正。イベント購読をWebView2初期化完了後に移動し、ハンドラにガードとtry-catchを追加
- **ブラウザからのURL D&D修正:** EdgeからのURLドラッグ＆ドロップが一部環境で動作しなかった問題を修正。ブラウザがFileDropとURLフォーマットを同時に送信する際、URLフォーマットを優先して処理するように変更
- **右ナビペイントグルボタンの常時表示:** 右ナビペインにアイテムがない場合でもトグルボタンを常に表示。ONにするとアイテムなしでも枠を表示
- **ナビペイン ドック切替クラッシュの修正:** ナビペインのドック固定ボタン押下時に、ヘッダーアクション要素が前の親から切り離されずに再追加されクラッシュする問題を修正
- **起動時AIグリーティングのUIブロック修正:** 起動パスでLazy&lt;AiService&gt;にアクセスしUIスレッドとデッドロックする問題を根本修正。AI設定チェックを静的フィールドに統一し、Lazy初期化を起動競合タイミングから遅延
- **プリセット適用時のクラッシュ修正:** レイアウトプリセット適用時にLoadFromLayout中のPropertyChanged→RebuildLayoutで論理子重複エラーが発生する問題を修正。LoadFromLayout中はRebuildLayoutを抑制
- **AIテーマ診断の応答途切れ修正:** MaxTokensを512→2048に増加し、Gemini等で応答JSONが途切れてパース失敗する問題を修正
