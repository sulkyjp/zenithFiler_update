...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.44.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.44.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.44.0_delta_from_0.43.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.44.0] - 2026-04-10 : ワーキングセットUI刷新・設定ウィンドウ統一・アプリフォント設定

### Added
- **起動時プリセット自動適用:** 設定画面でウィンドウ配置・ナビペイン・ワーキングセットの各プリセットを指定し、起動時に自動適用する機能を追加。作業環境の即時復帰が可能に
- **スクリーンキャプチャ (PRO):** PRO バッジのダブルクリックで範囲選択スクリーンキャプチャを起動。選択範囲を PNG 保存し、ファイルパスをクリップボードにコピー。右クリックメニューから保存先フォルダの変更も可能
- **ワーキングセット即時適用オプション:** ナビペインのワーキングセットビューにクリック即時適用モードを追加。ヘッダーの設定アイコンからプレビューモード / 即時適用モードを切り替え可能
- **アプリフォント設定:** Control Deck の Display カテゴリからアプリ全体のフォントファミリを変更可能に。システムフォント一覧から選択、即時反映

### Changed
- **ワーキングスペース一覧化:** ナビペインのワーキングスペース表示を3行カード形式から1行グリッド形式に変更。名前・日時・タブ数（A/B）の3カラム構成で一覧性を大幅に向上
- **ナビペイン設定ウィンドウの統一:** プロパティ・ワーキングセットの設定をポップアップからお気に入り設定と同じヘッダーバー付きウィンドウ形式に統一。全設定ウィンドウに OK ボタンと ESC キーキャンセルを追加
