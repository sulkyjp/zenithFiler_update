...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.4.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.4.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.4.2_delta_from_1.4.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.4.2] - 2026-07-04 : 起動速度を大幅に改善、クイックジャンプの検索範囲を見直し

### Changed
- **起動速度を改善:** マクロスタジオ追加でナビペインの部品が増えたことにより、起動直後にナビペインの低頻度ビュー（プロジェクトセット/よく使うフォルダ/ターミナル/マクロ等 14 種）の一括生成でUIスレッドが専有され、操作可能になるまでの体感が悪化していた回帰を解消。生成タイミングを画面操作が可能になった後まで遅らせ、1 件ずつ細かく生成する方式に変更した。あわせて、まだ生成されていないビューが選択された場合はその場で即時生成するフォールバックを追加し、表示のタイミング依存を解消した。ほか、テーマ一覧のスキャン結果を保持型キャッシュに変更して再スキャン頻度を削減し、ローカライズ・キーバインド・インデックスサービスの初期化をバックグラウンドで先行開始するようにした
- **起動速度をさらに改善（テキスト比較オーバーレイの遅延実体化）:** 2ファイルのテキスト差分比較パネル（DiffビューアとミニマップUI）を、これまで起動時に常に構築していた構成から、初めて比較を使ったタイミングでのみ構築する構成に変更した。テキスト比較機能を使わないユーザーは、起動時のその分の処理コストを支払わずに済むようになった
- **起動速度を大きく改善（クイックプレビューの遅延実体化）:** Spaceキーで開くファイルのクイックプレビュー画面（テキスト/構文ハイライト/画像/PDF/HTML/スプレッドシート表示など多数の部品で構成）を、これまで起動時に常に構築していた構成から、初めてプレビューを開いたタイミングでのみ構築する構成に変更した。プレビュー機能を一度も使わないセッションでは、起動時のその分の処理コストを支払わずに済むようになった
- **クイックジャンプの検索範囲を「お気に入り」「履歴」のみに戻した:** インデックス検索・よく使うフォルダ・よく開くタブ・最近のファイルまで候補に含めていたため範囲が広くなりすぎて使いにくかった問題を解消。候補ソースをお気に入りと参照履歴の2つに絞り、探しているフォルダを見つけやすくした
- **ステータスバーのマスコット表示のCPU負荷を軽減:** 画面のリフレッシュ毎に発火する描画ループを間引き、動きがある時は滑らかさを保ちつつ、立ち止まっているだけの待機中は描画頻度を抑えて常時CPU消費を削減した
