...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.0_delta_from_0.37.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.0] - 2026-03-19 : AI テーマ診断・生成・調整 + AI マニュアル

### Added
- **AI テーマ診断:** 生成AIが対話的に好みをヒアリングし、50テーマの中から最適なテーマを推薦する機能を追加。3〜5ラウンドの選択式質問で完結
- **AI テーマ自動生成:** 自然言語でイメージを入力するとAIが52色すべてを含むオリジナルテーマを生成。ライブプレビュー・修正リクエスト・保存して適用の一連のフローに対応
- **テーマ診断セクション分離:** Control Deck のテーマ設定で「テーマ診断」と「AIテーマ生成」を独立セクションに分割し、機能の違いを明確化
- **AI生成テーマカテゴリ:** テーマギャラリーに6つ目のカテゴリ「AI生成」を追加。AIで生成したテーマが自動的に専用カテゴリに分類される
- **AI テーマ調整:** テーマの右クリックメニューから「AIで調整」を選択し、自然言語で既存テーマの配色をAIに調整依頼できる機能を追加
- **テーマ削除:** AI生成テーマを右クリックメニューから削除できる機能を追加。適用中テーマの削除時は standard に自動切替
- **AI マニュアル:** マニュアルの内容を AI に自然言語で質問できる機能を追加。Lucene.NET で関連セクションを検索し、AI が回答を生成するチャット形式の UI。ナビペインのビューとしてもアクセス可能
- **AI マニュアル Markdown 表示:** AI マニュアルの回答を Markdown レンダリングで表示するように改善。見出し・太字・箇条書き・コードブロック等が整形表示され、可読性が向上
- **AI マニュアル バブル分割表示:** AI マニュアルの回答をセクション単位で複数のチャットバブルに分割して順次表示するように改善。長い回答の可読性が向上

### Changed
- **カスタマイズ済みテーマ表示:** カスタマイズ済みテーマカードの左端にアクセント色ストライプを表示し、視認性を向上
- **テーマカスタマイズ フラッシュハイライト:** ピックモード（スポイト）を廃止し、ダイアログからメインウィンドウの該当箇所をフラッシュ表示する方式に変更
- **テーマカスタマイズ 行クリック発火:** フラッシュハイライトの発火方法をScanEyeボタンから行クリックに変更。キー名・説明・余白クリックでフラッシュ、TextBox・スウォッチ・リセットボタンは既存動作を維持

### Fixed
- **AI テーマ診断:** 推薦結果の「おすすめ理由」テキストが1行で切れて全文表示されない問題を修正
- **テーマカスタマイズ 選択行表示:** カラーキー選択時に選択行のハイライトが維持されず視認できない問題を修正
