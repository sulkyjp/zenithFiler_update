...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.39.3.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.39.3_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.39.3_delta_from_0.39.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.39.3] - 2026-03-25 : プレビュー強化

### Added
- **コード構文ハイライト:** テキストプレビュー（Spaceキー）にAvalonEditによるシンタックスハイライトを導入。C#, XML, JavaScript, HTML, CSS, SQL, Java, C/C++, PHP等に対応。行番号表示付き
- **Markdownレンダリング:** .mdファイルのプレビューをMarkdigでHTML変換し、テーマ連動のスタイルで整形表示。見出し・リスト・テーブル・コードブロック等に対応
- **常時プレビューペイン:** ナビペインに「プレビュー」ビューを追加。ファイル選択に連動して画像・テキスト（構文ハイライト付き）・ファイル情報を自動表示
- **動画サムネイル:** .mp4/.mkv/.avi等の動画ファイルにサムネイル表示を追加。アイコンビュー・ホバーサムネイル・プレビューペインに対応
- **画像EXIFメタデータ表示:** プロパティペインで画像ファイル選択時に解像度・カメラ機種・撮影日時を自動表示
- **SVGプレビュー:** .svgファイルをWebView2（Space）またはShellサムネイル（プレビューペイン）で表示
