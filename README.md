...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.41.6.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.41.6_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.41.6_delta_from_0.41.5.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.41.6] - 2026-04-06 : 検索UI改善・常駐モード改善・AIフォルダ整理全件対応

### Added
- **Grep オプション UI:** Grep モード時に大文字小文字区別（Aa）と正規表現（.*）のトグルボタンを検索バーに表示
- **プリセットのクイック保存:** 検索条件を名前入力なしでワンクリック保存
- **プリセットの上書き保存:** 既存プリセットを現在の検索条件で上書き更新（コンテキストメニューから）
- **サイズフィルタの単位ドロップダウン:** サイズ入力欄に KB/MB/GB の単位選択プルダウンを追加
- **横バーアニメーション設定:** サイズ列の横バー伸長アニメーションを「起動時のみ」か「毎回」か選択可能に（設定 → 表示）
- **ウィンドウ配置プリセット:** ウィンドウの位置・サイズを名前付きで複数保存し、ワンクリックで切替。ステータスバー左側のプルダウンとトレイアイコンのサブメニューから利用可能。右クリックでリネーム・削除

### Fixed
- **バッチファイルの「テキストで開く」修正:** .bat/.cmd ファイルを「テキストで開く」で実行されてしまう問題を修正。edit Verb を使用し、標準テキストエディタで開くよう変更
- **AIフォルダ整理の全件対応:** フォルダ内の全ファイルをAIに送信するよう改善（旧: 200件超は先頭50件のみ）。アクション数上限を撤廃し、トークン上限到達時は自動継続で全件を処理

### Changed
- **常駐モードの改善:** ×ボタンやAlt+F4で閉じた際、ウィンドウを非表示ではなく最小化に変更。タスクバーにアイコンが残り「まだ起動中」であることが明確に。タスクバー右クリックメニューに「終了」を追加
- **自動更新の安定性向上:** Worker のバージョン確認を GitHub API 依存から D1 永続キャッシュに移行。リリース時に GitHub Actions から Worker へ自動通知する方式に変更し、rate limit による失敗を完全に解消
