...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.41.8.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.41.8_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.41.8_delta_from_0.41.7.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.41.8] - 2026-04-07 : プリセット強化・ショートカット追加・XPランキング改善

### Added
- **ウィンドウ配置プリセットのディスプレイ対応:** 保存時にディスプレイ番号を記録し、復元時に同じディスプレイに配置。選択UIやトレイメニュー、JumpList にもディスプレイ番号を表示
- **ウィンドウ配置・ナビペイン配置プリセットのショートカット:** ウィンドウ配置プリセット（Ctrl+Shift+L）とナビペイン配置プリセット（Ctrl+Shift+P）のキーバインドを追加。設定画面からカスタマイズ可能
- **ワーキングセット クイックアクセス:** ツールバーにワーキングセットの即時切替アイコンを追加。ポップアップメニューから保存済みセットをワンクリックで適用可能。ショートカット（Ctrl+Shift+W）にも対応

### Changed
- **XPランキングのデフォルトを匿名アップロードに変更:** 初回起動時のアップロードモードを「しない」から「匿名でアップロード」に変更。個人情報は収集しない旨の注意書きを追加
- **内部認証方式の刷新:** ID ベースの認証に完全移行し、メールアドレス入力を廃止
