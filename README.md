...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.7.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.7_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.7_delta_from_0.40.6.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.7] - 2026-03-27 : チャレンジ / 実績システム

### Added
- **チャレンジ / 実績システム:** 10種類のチャレンジ（お気に入り登録・テーマ変更・AI整理・クイックジャンプ等）を達成するとトースト通知で祝福。タイトルバーのトロフィーアイコンまたは設定画面からチャレンジページに直接アクセス。達成状況はカラーバッジアイコン + プログレスバーで視覚化
