...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.0.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.0.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.0.2_delta_from_1.0.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.0.2] - 2026-06-06 : インデックス検索の登録が毎起動で消える重大バグを修正／インデックス詳細設定の時刻表示を修正

### Fixed
- **インデックス検索の登録フォルダが毎起動で消える不具合を修正:** 起動時、インデックス検索ビューの初期化が登録の読み込み（LoadPaths）より前に保存処理（PersistTargets）を実行しており、登録一覧がまだ空の状態で settings.json を空配列で上書きしていた。直後の読み込みが空を読むため、登録が毎回の起動で失われていた（クラウド/USB/ネットワークの有無に関係なく発生）。初回読み込みが完了するまで保存を抑制するよう修正し、根本的に解消した。あわせて、登録の追加/削除を即時保存して、直後のクラッシュ・強制再起動・USB取り外し・自動更新でも失われないようにし、未マウントのフォルダ（クラウド/ネットワーク/リムーバブル）も登録として保持して一覧に「現在アクセスできません」と表示し、再びアクセス可能になれば自動でインデックス作成を再開するようにした
- **インデックス詳細設定の「時刻」欄に内部コードが表示される不具合を修正:** スケジュール時刻の選択ボックスに「ScheduleHourOption { Value = -1, Display = 制限なし }」のように内部表現がそのまま表示されることがあった。選択肢が常に表示名（「制限なし」「9:00」など）で表示されるようにした
