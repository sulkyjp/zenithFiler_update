...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.8.5.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.8.5_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.8.5_delta_from_1.8.4.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.8.5] - 2026-07-16 : テーマストアの絞り込みをタグ方式に刷新し、AIテーマ生成でタグを保存。ストアカードのデザインも刷新

### Added
- **AIテーマ生成時にタグを保存するように変更（ストアのタグ絞り込みの下地）:** これまでイメージ選択チップ（明るさ・色の傾向・鮮やかさ・雰囲気・情景の計73種）はAIへの指示にのみ使われ、生成後は情報が捨てられていた。チップを選択して生成した場合はその選択トークンをそのままテーマJSONの `Tags` に保存し、チップを使わず自由文だけで生成した場合はAI自身に73語彙の中から3〜6個を自己申告させて保存するようにした（語彙にない語は破棄するホワイトリスト検証つき）。カテゴリ（Standard/Professional等5種）は雰囲気の大分類として残しつつ、より具体的な検索の軸としてタグを新設した

### Changed
- **テーマストアの絞り込みをカテゴリのドロップダウンからタグのファセットに刷新:** 5カテゴリのドロップダウンは、実際には0件になる選択肢（「テーマ作成」「ストア」）が混ざっていたうえ、AIの自己申告カテゴリだけでは検索の軸として弱いという課題があった。ドロップダウンを廃止し、「明るさ」「色の傾向」「鮮やかさ」「雰囲気」「情景」の5グループボタンを常設、押下すると配下のキーワードボタンが展開する省スペースなファセットUIに変更した。キーワードは現在ストアに実在するタグのみを出現件数付きで表示し、単一選択・再クリックで解除できる。グループボタンはアイコン付きのピル型＋展開シェブロンでモダンな見た目にし、絞り込み中は「すべて解除」ボタンで一括解除できる。検索ボックスもタグの表示名にヒットするように拡張した
- **テーマストアのカードデザインを刷新（レイアウトプレビュー要素を削除し、ダウンロードボタンを常時表示に変更）:** カード中央にあったレイアウトのミニプレビュー要素を削除してカードの縦の長さを圧縮し、一覧全体の見通しを改善した。これまでインストール済みのテーマでは非表示になっていたダウンロードボタンを常に表示するように変更し、インストール済みの場合はボタンを非活性（減光）にして表記を「ダウンロード済」に切り替えるようにした。あわせて、タイトルに重なって視認性を下げていた「インストール済み」バッジを廃止（ボタンの状態で判別できるため）し、テーマ名がカード幅いっぱいに表示されるようにした。カードの高さを揃え、ダウンロードボタンをどのテーマでもタイル最下部の同じ位置に固定配置することで、一覧全体の統一感を高めた。全体として、モダンで統一感のあるタイルデザインに整えた

### Fixed
- **テーマの無料利用回数を使い切っても、既定テーマにはいつでも無料で戻せるようにした:** これまでは無料の変更回数を使い切ると、既定テーマ（standard）へ戻す操作までブロックされ、見づらいテーマから抜け出せなくなることがあった。既定テーマへの復帰を回数の消費・判定の対象外にし、いつでも読みやすい既定の見た目に戻せる安全弁を用意した
