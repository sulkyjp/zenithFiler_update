...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.39.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.39.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.39.0_delta_from_0.38.9.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.39.0] - 2026-03-24 : テーマストア + AIグリーティング強化

### Added
- **テーマストア基盤:** テーマストアサービス（ThemeStoreService）を追加。ストアからのテーマ一覧取得・ダウンロード＆インストール・アップロード・評価機能の通信基盤を実装。ユーザーIDの自動生成・永続化、テーマ情報へのストア評価データ（平均評価・件数・DL数）の保持にも対応
- **テーマストアUI:** Control Deck にテーマストアパネルを追加。コミュニティテーマの閲覧・カテゴリフィルタ・検索・ダウンロード＆インストール・★評価・マイアップロード統計表示に対応。テーマ一覧カードにストア評価情報（★平均・件数・DL数）を表示。テーマカード右クリックから直接ストアにアップロード可能
- **テーマストアWorker:** Cloudflare Worker + D1 によるテーマストアバックエンドを構築。テーマアップロード・評価（upsert）・DLカウント・マイアップロード統計・自分の評価取得APIを実装。ストア表示時にユーザーの既存評価を自動復元
- **テーマ作者名:** ライセンス登録時にユーザー名を入力可能に。AI生成テーマやアップロード時の作者名として使用される。ライセンスページ・テーマページで随時変更可能。Free版ユーザーは "Free User" が自動設定
- **ビルトインテーマ制御:** デフォルト50テーマにシールドアイコンを表示し、アップロード・ダウンロード対象から除外。カスタム済みビルトインテーマは名前変更後にアップロード可能
- **テーマカードUX改善:** 右クリックでテーマが適用されない改善、カード上のクイックアップロードボタン追加、アップロード時のGlowBar進捗表示
- **テーマストアソート:** ストア一覧にソート順プルダウンを追加。新しい順・評価順・DL数順で切り替え可能
- **テーマストアプレビュー:** ストアのテーマカードを左クリックでテーマを一時適用して試せるように。右クリックメニューから「元に戻す」「ダウンロード」が可能。ストアから離脱すると自動で元のテーマに戻る
- **テーマ適用高速化:** カラーパース結果のキャッシュ導入、ストアプレビュー時のファイルI/Oスキップ、SolidColorBrushのFreeze適用により、テーマ切替速度を大幅に改善
- **テーマ再アップロード:** 同じ作者が同名テーマを再アップロードすると、テーマJSONのみ上書き（DL数・評価は保持）。別ユーザーの同名アップロードは拒否
- **アップロード済みテーマの削除:** マイアップロードのカードを右クリック→「ストアから削除」で自分のテーマを削除可能
- **AIグリーティングのランダム性強化:** プロンプトにスタイル・キーワード・シードをランダム注入し、同じ時間帯でも毎回異なるメッセージを生成
- **グリーティングトーン調整:** プロパティのグリーティングに「事務的に」「寄り添って」ボタンを追加。押すたびに段階的（7段階）にトーンが変化し、現在のレベルをドットで表示

### Changed
- **フルアップデートボタン追加:** アップデート検出時、自動アップデートに加えて「フルアップデート」ボタンを表示。動作不良や言語設定未反映時にフルアップデートで解消できる旨の案内メッセージも併記

### Fixed
- **AIウェルカムメッセージの非同期化:** プロパティビューのAIグリーティング生成を完全非同期化し、応答待ち中もアプリ操作が可能に。グリーティング表示中はアイテム選択による表示切替を抑制し、自動非表示完了後に最新の選択状態を表示
- **AI モデル指定の修正:** 設定画面で指定した AI モデルがハードコードリストにない場合に旧モデルへフォールバックされる問題を修正。疎通テスト済みモデルをそのまま使用するように変更
- **マイアップロードカード表示の修正:** マイアップロードのテーマカードに説明文・作者名・カラーチップが表示されていなかった問題を修正。評価分布バーにラベル・件数・比率バーを追加
- **設定ファイル保存エラーの修正:** ナビペイン幅計算で発生するInfinity/NaN値がJSONシリアライズに失敗する問題を修正。JsonNumberHandling.AllowNamedFloatingPointLiteralsを追加
- **AIプロバイダ/モデル不整合の修正:** プロバイダ切替時にモデル名がグローバル共有されていたため、別プロバイダのモデルで401エラーが発生する問題を修正。リクエスト時にモデルの互換性を検証し、不整合時はデフォルトモデルにフォールバック
- **AI分析のJSON修復改善:** MaxTokensで途切れたAI応答のJSON修復ロジックを改善。文字列リテラル途中での切断を検出し、未閉じ文字列とその所属プロパティを安全に除去してからブラケットを補完
- **ナビペイン幅の手動制御不能の修正:** 幅自動設定ON後にGridSplitterで手動幅変更が効かなくなることがあった問題を修正。ColumnDefinitionのStyle Setterバインディングをコードビハインド直接制御に変更し、WPFのDependencyProperty優先順位競合を解消
