...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.43.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.43.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.43.0_delta_from_0.42.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.43.0] - 2026-04-09 : Stickman スマート選択エンジン・歩行インターリーブ・実績報酬システム

### Added
- **Stickman 性格設定:** おだやか・やんちゃ・きまぐれ・はたらきものの4種類から相棒の性格を選択可能に。行動パターンの出現率とリアクション感度が性格に応じて変化
- **Stickman スマート選択エンジン:** 行動選択を完全ランダムから重み付きスマート選択に改修。basic/special カテゴリ分類、Tier 重み付け（高 Tier ほどレア）、リピート減衰による自然な出現頻度制御
- **Stickman 性格別速度:** 性格に応じて移動・手足アニメーション速度が変化。おだやか（0.7x）、きまぐれ（標準）、はたらきもの（1.1x）、やんちゃ（1.3x）
- **WallClimb 改良:** 画面左端・右端に到達してから壁登りを開始するように改修。1回目と2回目で異なる端に向かい、登り高さも拡大
- **Stickman 実績解放報酬一覧:** Stickman ページにチャレンジと同じ石板タイルレイアウトで15カテゴリ × 10 Tier = 150の実績報酬一覧を追加。アンカーリンクによるカテゴリジャンプに対応
- **実績報酬の行動パターン名を英語に統一:** 全言語で報酬行動パターン名を英語表記に変更。名前と説明の視認性を向上
- **チャレンジ・報酬タイルの高さ統一:** 同一カテゴリ内のタイルが最も高いタイルに合わせて自動で高さ揃えされるように改善
- **ナビゲーション報酬行動パターン（10種）:** ナビゲーション実績 Tier 1〜10 の報酬として、おそるおそる歩く〜遍歴の王の堂々行進まで10種の専用 Stickman 行動パターンを追加。Tier に応じた動きの複雑さと犬との連携演出
- **報酬タイルのホバープレビュー:** Stickman 報酬タイルにマウスホバーすると、行動パターンのアニメーションプレビューがポップアップで表示される
- **報酬タイルの ON/OFF チェックボックス:** 達成済み報酬タイルの右上にチェックボックスを追加。行動パターンの有効/無効を個別に切り替え可能
- **報酬行動の実績解放制御:** `stickman/rewards/` フォルダの報酬行動 JSON に `unlock` フィールドを追加。対応チャレンジ未達成の場合、行動がエンジンに登録されない
- **既存行動パターンの報酬化（12種）:** 既存の特殊行動（run, dance, jump, wave 等12種）を実績報酬に移行。基本行動を idle/walk/sit/sleep/look_around の5件に絞り、実績達成で行動が増えていく成長感を演出
- **チャレンジタイルに報酬インジケータ:** 各チャレンジタイルの右下にギフトアイコンを追加。ホバーで解放される報酬名を確認可能（未達成時は ???）
- **チャレンジタイルのツールチップ高速化:** 表示遅延を 300ms→100ms に短縮、タイル間移動時の再表示遅延を 0ms に設定。隣接タイルへの横移動でツールチップが即座に切り替わるように改善
- **Stickman 歩行インターリーブ:** 行動と行動の間に必ず歩行移動フェーズを挿入。ステータスバー幅の一定割合を歩いてから次の行動に移る仕組みで、にぎやかすぎる印象を抑制。歩行距離は性格に応じて変化（おだやか＝長く、やんちゃ＝短く）

### Fixed
- **Stickman 着地モーション後のスライド修正:** 設定画面⇔メイン画面切替時の着地モーション後、手足が動かずスライドする問題を修正。着地完了後に行動を新規開始するように変更
- **Stickman 行動パターンの無効化が反映されない問題を修正:** チェックを外した行動がそのまま実行される問題を修正。設定変更が即座にエンジンに反映されるように
