...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.13.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.13_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.13_delta_from_0.40.12.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.13] - 2026-03-28 : 二つ名バナー強化 + 1画面モードフリーズ修正 + メモリ削減

### Added
- **チャレンジ経験値（XP）表示:** チャレンジページに区分別係数で計算した経験値を表示。日常操作（×1〜3）からカスタマイズ（×15〜20）まで、こだわり度に応じた配点で積み上がる
- **二つ名ウェルカムグリーティング:** 起動時にチャレンジで獲得した二つ名で「おかえりなさい、○○殿。」と表示。AI 有効時はその後パーソナライズされたウェルカムメッセージに差し替え
- **チャレンジシステム全10言語ローカライズ:** 100チャレンジ（10区分×10レベル）の称号名・説明文を全10言語に翻訳。ファンタジーRPG調のトーンを各言語で再現
- **二つ名バナーのバックストーリーツールチップ:** タイトルバーの二つ名バナーにホバーすると、AI 生成のバックストーリーをリッチなツールチップで表示

### Changed
- **メモリ使用量の大幅削減（1GB→300MB目標）:** Lucene RAMBufferSizeMB を 16→4MB に削減、Auto モードの単発 Commit を1秒デバウンスでバッチ化（GC 圧力を大幅軽減）、インデックス完了時に Writer バッファをフラッシュ、GC ヒープハードリミット 300MB を設定、非アクティブ時の GC 遅延を 60→15秒に短縮
- **静的キャッシュのメモリ上限追加:** カラーパースキャッシュ（500件上限）、テーマ JSON プリロード（10件上限）、ディレクトリツリーイベントスロットル（500件 + TTL 60秒）に上限を設定。インデックス完了時に SearcherManager の古いリーダーも解放

### Fixed
- **1画面モードで起動不能になる問題を修正:** Windows Update 後、1画面レイアウト（右ペイン Collapsed）で画面外退避すると DWM がウィンドウ描画を完了できずフリーズしていた。起動時は2画面で描画し ContentRendered 後に1画面に切替。設定ロード時の座標サニタイズと終了時の Minimized フォールバック保存も追加
- **チャレンジページ離脱後のパフォーマンス劣化を修正:** チャレンジ画面の AI 生成タスク（二つ名・バックストーリー）がカテゴリ離脱後もバックグラウンドで実行し続け、メモリ・スレッドが回復しない問題を修正。離脱時に AI タスクをキャンセルし、ビジュアルツリー（100ノード）も解放するように改善
- **ナビペインのアイコンクリック時にクラッシュする問題を修正:** レイアウト再構築時にビジュアルツリー未確立のビューを検出できず「既に別の要素の論理子です」例外が発生していた問題を修正。ツリー走査をロジカルツリーベースに変更
- **チャレンジページ切り替え時のフリーズを修正:** 100ノードの ObservableCollection を逐次 Add/Clear するたびに描画が発火し UI スレッドがブロックされていた問題を修正。ローカルで組み立ててから一括差し替えする方式に変更し、離脱時の不要な Clear も削除
