...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.8.4.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.8.4_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.8.4_delta_from_1.8.3.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.8.4] - 2026-07-15 : 社用PC（Box大量同期環境）でフォルダ移動のたびに数秒固まる問題を修正

### Fixed
- **社用PC（Box大量同期・EDR/DLP環境）でフォルダ移動のたびにアプリ全体が数秒固まることがある問題に対処:** 実機ログ解析で、Box Drive のフォルダ存在確認がタイムアウト（2秒）するほど応答が遅い環境で、連続したフォルダ移動のたびに新規の存在確認I/Oが積み重なっていたことを確認した。タイムアウトで呼び出し元へは制御を返しても、裏側の実I/Oはそのまま完走まで動き続ける仕様だったため、連打的なナビゲーションでブロッキングスレッドが積み上がっていた。同一パスへの重複した存在確認を1本のI/Oに合流させるよう修正し、ナビゲーション連打時の負荷積み上がりを抑制した。あわせて、Box領域のアイコン読み込み時に行っていたフォルダのスタブ生成トリガー（軽量列挙）が、Box Drive側の応答遅延時にアイコン読み込みの直列キュー全体を巻き込んで止めてしまっていた箇所を、結果を待たない非同期実行に変更した
