...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.4.3.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.4.3_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.4.3_delta_from_1.4.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.4.3] - 2026-07-05 : ナビペインの切替を高速化・安定化

### Changed
- **ナビペイン（左右サイドバー）のビュー切替・開閉を高速化:** ビューを切り替えるたびにサイドバー内部のアイコンバー・ヘッダー・区切り線まで含めて作り直していた構造を見直し、切り替わった中身だけを差し替える方式に変更した。あわせて、切替のたびに毎回行っていた一覧の再読み込み（履歴・ドライブ・よく使うフォルダ・最近のファイル・検索履歴）を「初回表示時と、非表示中にデータが変わっていた場合のみ」に絞り、連続切替でもキビキビ反応するようにした
- **ナビペインの表示/非表示トグル時のトースト通知を廃止:** 開閉のたびに「左ナビを表示しました」等の通知が出て操作のテンポを削いでいたため、通知を出さないようにした（切替結果は画面を見れば分かるため）

### Fixed
- **サイドバーの自動隠しモード切替後に一部の操作が反応しなくなる問題を修正:** 自動隠しの ON/OFF を切り替えた後、ビュー切替への追従・アイコンの D&D 表示・言語切替への追従が効かなくなることがあった内部的な購読切れを解消した
- **サイドバー下部ヘッダーのピン留めボタンが消える問題を修正:** ヘッダーの再構築時に、自動隠しモードで使うピン留めボタンまで取り除いてしまい、以後表示されなくなっていた
- **自動隠し OFF へ切替直後に空のサイドバー枠が一瞬表示されることがある問題を修正:** 切替前に予約されていた表示タイマーが遅れて発火した場合でも、空のオーバーレイを表示しないようにした
