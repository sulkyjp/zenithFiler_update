...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.25.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.25_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.25_delta_from_0.40.24.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.25] - 2026-03-31 : タブ複数行固定化・チャレンジ演出強化

### Added
- **Tier 10 王冠パルスアニメーション:** チャレンジで最高 Tier に到達・完了した王冠アイコンが緩やかに明滅し、輝いている印象を演出
- **フィードバックボタン:** タイトルバーのアイコン群にフィードバックボタンを追加。クリックでアプリ設定のフィードバックページを直接開ける
- **AIマニュアルボタン:** タイトルバーにAIマニュアルボタンを追加。マニュアルボタンの横からAIマニュアルを直接開ける

### Fixed
- **タブ複数行表示時の行入れ替え修正:** 複数行タブモードで上段のタブを選択すると下段に自動移動する WPF 標準動作を修正し、タブ位置を常に固定化

### Changed
- **チャレンジページの軽量化:** 不要な初期 DropShadowEffect の除去によりレンダリング負荷を軽減
- **王冠アイコンの表示条件変更:** チャレンジの王冠アイコンを Tier 10 到達済みの区分のみに表示するよう変更
- **達成済みチャレンジの石板グロー:** 達成済み Tier カードの石板アイコンが常時グローするよう変更。ホバー時はさらに強調
