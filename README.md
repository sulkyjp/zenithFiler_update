...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.18.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.18_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.18_delta_from_0.40.17.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.18] - 2026-03-29 : チャレンジ演出強化 + マルチモニタ DPI 修正

### Added
- **チャレンジカードにホバー演出を追加:** マウスホバー時に石板アイコン外周にゴールドのグロウエフェクト。CircleEase による重厚な遷移。カードホバーで石板のみ発光する上品な演出
- **チャレンジカードにカスタム画像アイコンを対応:** images/challenges/{id}.png を配置するだけで Lucide アイコンから自動切替。フォールバック対応

### Fixed
- **チャレンジのクイックジャンプショートカット表記を修正:** Ctrl+G → Ctrl+P（アプリ標準）に修正（全10言語）
- **チャレンジ実績名が長い場合に改行されるよう修正:** 固定幅ノード内で TextWrapping を有効にし、アイコンを上部に配置。英語等の長い実績名でも表示が崩れないよう対応
- **マルチモニタ DPI 環境でウィンドウサイズ・位置が再起動のたびにずれる問題を修正:** 起動時の画面外退避で座標を (-100000, -100000) に設定していたため、プライマリモニタ（DPI 125%）の領域に入り PerMonitorV2 の DPI リスケールが発火。退避時に Y のみ画面外にずらすことで DPI 変更を回避。位置は SetWindowPos + NativeBounds で正確に復元、サイズは WPF プロパティで復元するよう分離
