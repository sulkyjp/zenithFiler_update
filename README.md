...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.7.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.7.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.7.2_delta_from_1.7.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.7.2] - 2026-07-10 : マクロ機能UIの抜本刷新（一覧専用ナビペイン・マクロスタジオ拡張）

### Changed
- **マクロ機能のUIを抜本的に見直し、ナビペインを「一覧＋実行専用」・マクロスタジオを「作成・編集の唯一の場所」に再編:** 従来のナビペインのマクロビューはコードエディタ・GUIビルダー（ステップ積み上げ式）・保存マクロ一覧の3セクションを縦に詰め込んでおり、ペイン幅では文言やボタンが見切れて実用に耐えなかった。ナビペインは保存済みマクロの一覧（名前・トリガーバッジ・実行ボタン、ダブルクリック実行、右クリックで実行/スタジオで編集/複製/削除）のみに縮小し、作成・編集はすべてマクロスタジオ（独立モードレス窓）に集約した。旧・直列GUIビルダーは廃止し、上位互換のノードグラフに一本化（既存のビルダー形式保存マクロは読込時に自動でノードグラフへ変換）。マクロスタジオは「ライブラリ列（保存済み一覧・選択で編集対象切替）／ノード・コードのタブ切替編集面／プロパティ列（名前・説明・トリガー設定・実行結果）」の3列構成に拡張し、保存は編集中マクロの上書き更新（従来は毎回新規保存されていた）に変更。手書きJS・記録直後のマクロはコードタブで直接編集でき、ノードグラフ由来のマクロはコードタブが読み取り専用の生成プレビューになる（編集はノードタブで行い、手編集との食い違いによる上書き事故を防ぐ）。未保存の変更がある状態でライブラリ選択切替・新規作成・ウィンドウを閉じる操作をすると破棄確認ダイアログを表示する。ステータスバーのマクロ操作は「記録」「一覧」の2ボタンに整理し、単独の「実行」ボタン（現在エディタのスクリプトを実行）は編集面の消失に伴い廃止。一覧ポップアップの「現在のスクリプトを保存」は「＋ 新規マクロ」（マクロスタジオを新規マクロ状態で開く）に、右クリック「編集（エディタに読込）」は「スタジオで編集」に置き換えた。記録（⏺）を停止すると、生成されたコードを確認・保存できるようマクロスタジオがコードタブで自動的に開く。あわせて、従来ハードコードだった新UI文言・通知メッセージを10言語ローカライズキー（`macro.*`、58キー）へ移行した
