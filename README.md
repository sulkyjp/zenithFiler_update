...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.8.3.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.8.3_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.8.3_delta_from_1.8.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.8.3] - 2026-07-15 : テーマストアの一覧・カードデザインを刷新、設定画面全ページに「トップに戻る」ボタンを追加

### Added
- **設定画面（オプション）の全ページに「トップに戻る」ボタンを追加:** 基本設定・テーマ・ストア・生成AI・統計・チャレンジなど、縦に長い設定ページを一定量スクロールすると右下に表示され、クリックでそのページの先頭まで一瞬で戻れるようにした。現在表示中のページ（AIタブ内のサブタブ切替も含む）を自動的に検出して追従するため、ページ追加時にも個別対応は不要な作り

### Changed
- **テーマストアのカードデザインをテーマギャラリーと統一し、テーマ名・説明をできる限り全文表示するように変更:** カード自体をそのテーマの配色（背景色・サイドバー色・アクセント色・本文色）で塗るミニプレビュー方式に刷新し、テーマギャラリーと統一感のある見た目にした。テーマ名は2行まで、説明はより多くの行数まで折り返して表示し、それでも収まらない場合はカードにカーソルを合わせるとモダンなデザインのツールチップで全文（テーマ名・説明・作者）を確認できるようにした
- **テーマストアの一覧から同梱デフォルトテーマ（50個）を除外:** 「新しい順」等のソートに、アプリに最初から入っているテーマがノイズとして混ざっていた問題を修正し、新規に登録されたテーマのみが表示されるようにした
- **AIテーマ生成ダイアログのスクロールバー位置をダイアログ右端に寄せて調整:** チップ一覧のスクロールバーがダイアログの枠から離れた位置に表示され、見た目に違和感があった問題を解消した
- **AIテーマ生成のイメージ選択チップを36個から73個に大幅拡充:** 「カッコいい」「暴力的」「荒々しい」「危険な香り」といった、これまで選択肢のなかった方向を含む雰囲気16種（ダークヒーロー・危険な香り・攻撃的・荒々しい・パンク・ミリタリー・ホラー・ゴシック・ぬくもり・エレガント等）と情景20種（戦場・血の月・地下迷宮・廃墟・溶岩・火山・古い図書館・桜・紅葉・スチームパンク等）を追加した。あわせて色の傾向の「赤・ピンク系」は正反対の印象を持つ組み合わせだったため「赤系」「ピンク系」に分離。穏やかな方向からエッジの効いた方向までグラデーションで並ぶよう並び順も整理し、あらゆるイメージの伝達に対応できるようにした
