...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.26.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.26_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.26_delta_from_0.40.25.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.26] - 2026-04-01 : UI統一・起動高速化・テーマ修正

### Added
- **タブ管理ビューの D&D ガイド線:** ナビペインのタブ管理でタブをドラッグ＆ドロップする際、挿入位置を示す水平ガイド線を表示
- **ウィンドウ復帰メニュー:** タスクバーの右クリック JumpList、タイトルバーのシステムメニュー、および常駐モード時のトレイアイコンメニューに「メイン画面に移動」「カーソルのある画面に移動」を追加。画面外にウィンドウが出た際のリカバリに対応
- **Ollama (ローカル LLM) プロバイダー対応:** API キー不要でローカル実行可能な Ollama プロバイダーを追加。ワンクリック自動インストール、推奨モデルのダウンロード管理、サーバーの自動起動・停止に対応。AI 設定の Ollama タブにステータス表示・エンドポイント設定・インストール済みモデル一覧・推奨モデル取得を配置
- **AI 設定ページのタブ化リファクタ:** AI 設定を概要・Ollama・Gemini・Azure OpenAI・OpenAI・Claude の6サブタブに再構成。概要タブに自動機能別トグル（グリーティング・リネーム提案・二つ名生成・バックストーリー生成・バックアップサマリー）を追加
- **レベルアップ・実績達成時のバナーグロー演出:** レベルアップ時にレベルバナー、実績達成時に二つ名バナーの周囲がアクセントカラーでグローパルスする演出を追加

### Changed
- **AI 設定ページから AI マニュアルセクションを削除:** アプリヘッダーからアクセス可能なため、設定ページ内の重複を解消
- **非活性サブタブの視覚化改善:** AI 設定の非活性プロバイダタブを Opacity 35% でグレーアウト表示し、選択不可であることを明確化
- **タブ管理ビューの画面モード対応:** 1画面モード時はAペインのタブのみ表示し、2画面モード時はA/B両ペインを表示するように切替。サマリー表示も画面モードに連動
- **起動速度の改善:** ペインテーマ適用時の不要な Storyboard 再構築をスキップ、ContentRendered 後のレイアウト再構築を遅延実行、キーバインド構築・タブ高さ同期をウィンドウ表示後に遅延、テーマのカスタムオーバーライドをバックグラウンドで事前キャッシュ
- **クイックジャンプの改善:** ウィンドウ枠外クリックで閉じるよう対応、マウスカーソル位置付近にダイアログを表示

### Fixed
- **テーマカスタマイズ後にテーマ切替が正常に動作しない問題を修正:** ライブプレビューがトップレベルリソースに直接書き込んでいたため、テーマ切替時に古い値が残留していた。テーマ辞書（MergedDictionary）に書き込むよう修正
- **レベルバナー・二つ名バナーの色がテーマと同期しない問題を修正:** LevelBadgeColor / TitleBadgeColor がテーマのデフォルト色定義に含まれておらず、テーマ切替時に更新されなかった。テーマカスタマイズ画面にも追加
- **言語設定の選択済みスタイルを統一:** 他のページの選択済みボタンと同じ OptionSelected スタイルに変更
- **コンテキストメニューのスタイル完全統一:** テーマ設定・テーマストアのコンテキストメニューでホバー時にフォント色が白に変わらなかった問題を修正。ストアメニューへの ContextMenuItemStyle 適用漏れも修正
- **プロパティビューの定期提案が停止する問題を修正:** 推薦候補が一時的に0件になるとタイマーが永久停止していた問題を修正。候補なしでもタイマーを継続し、カウンター取得をファイル再読込からメモリ参照に変更
