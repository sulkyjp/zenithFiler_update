...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.0.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.0.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.0.0_delta_from_0.50.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.0.0] - 2026-05-31 : 正式版リリース — インデックス検索の登録設定の永続化を堅牢化／右ナビペイン非表示時の空白列を修正

### Fixed
- **インデックス検索の登録設定が再起動で失われる問題を修正:** 検索対象フォルダの登録・削除・ロック状態の変更を、これまでアプリの正常終了時（`MainWindow_Closing`）にのみ保存していたため、強制再起動・クラッシュ・ログオフなど正常終了処理が走らない終了をすると、その回に行った登録がすべて失われていた。スコープ選択やアイテム別設定と同様に、登録・削除・ロック変更の都度デバウンス保存するよう変更し、加えてセッション終了（ログオフ／シャットダウン／再起動）時にも未保存分を確実にフラッシュするようにして、登録が確実に永続化されるようにした
- **右ナビペイン非表示時に右側へ空白列が残る不具合を修正:** 右ナビペインを非表示にしているのに、内側ビューの幅自動リクエスト（プロパティ等「幅自動」ビューの再測定やレイアウト復元などで発火）によって列幅だけが復活し、中身のない空白列が右側に残ることがあった（以前から間欠的に再発）。ナビペインの幅自動ハンドラが可視状態（`IsRightNavVisible` / `IsSidebarVisible`）を確認せずに列幅を書き戻していたのが原因。非表示時は列幅 0 を維持するよう修正（左ナビペインの同型の潜在不具合も併せて修正）。再表示時は元の幅で正しく復元される
