...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.4.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.4.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.4.0_delta_from_1.3.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.4.0] - 2026-06-20 : マクロスタジオ（2D ノードでマクロを組む）を追加、GUI ビルダーを刷新

### Added
- **マクロを 2D ノードグラフで組める「マクロスタジオ」を追加:** ナビペインのマクロビューの「⛶ 広い画面」ボタンから、広いキャンバスでパズルのように部品を組み合わせてマクロを作れるモードレス窓を開けるようにした（開いたまま裏でフォルダ閲覧・操作も可能）
  - **部品をドラッグ&ドロップ:** 左の部品パレットから操作ブロックをキャンバスへドラッグして好きな位置に配置し、ブロック端の「穴」どうしを線でつないで実行順を決められる（クリックでも追加可能）。各部品はホバーで用途の説明を表示する
  - **入力支援（選ぶだけ）:** 各ブロックの入力欄は種別に応じて「参照…（フォルダ選択）」「現在フォルダの内容から選ぶ候補リスト」「トークン挿入」「よく使う値のプリセット」を備え、手入力を最小限にした
  - **条件分岐:** 「条件分岐」ブロックは「はい / いいえ」の 2 つの出口を持ち、条件（選択がある／アイテムがある／確認 OK／ファイルが存在／カスタム式）で処理を分けられる（実行コードの if/else を自動生成）
  - **部品の種類を拡充:** 新規タブ・親フォルダへ・ホームへ・一覧更新・待機・ゴミ箱へ削除（復元可能）・AI 整理・連番リネーム などのブロックを追加した
  - **カード幅の調整・配置の保存/復元:** 各ノードカードは右端のグリップで幅を変えられる。組んだグラフ（配置・幅・接続・分岐）を保存でき、保存済みマクロの「🧩 ノードで開く」から配置ごと再編集できる
  - 配色はアプリのテーマに追従する（ライト / ダーク両対応）
- **マクロの GUI ビルダーを大幅に使いやすくした:** コードを書かずに操作を組めるビルダーを刷新し、フォルダ名やパスを固定値で手入力しなくても済むようにした
  - **動的トークン:** フォルダ名やメッセージに「今日の日付」「年月」「時刻」「現在のフォルダ名」「選択中のアイテム名」を差し込めるようにした（各入力欄の「{ }」ボタンから挿入。実行時に値へ展開）。例: フォルダ作成で `売上_{date:yyyy-MM}` と入力すると、実行時に `売上_2026-06` フォルダを作成する
  - **フォルダ参照ボタン:** 移動先・コピー先・移動先パスを「参照…」ボタンでフォルダを選んで入力できるようにした（手入力不要）
  - **対象を一覧から選択:** リネーム・移動・コピーの対象を、現在フォルダの内容からドロップダウンで選べるようにした（自由入力・トークンも可）
  - **ライブプレビューと自動反映:** 各ステップの生成コードをその場で表示し、編集すると自動でエディタのスクリプトへ反映されるようにした（「⚙ 生成」を押し忘れても反映される）
  - **保存マクロをビルダーで再編集:** ビルダーで作成したマクロは、保存後も「🧩 ビルダー」ボタンから読み込んでステップ単位で編集できるようにした

### Changed
- **ステータスバーのマクロプルダウンから保存・削除できるようにした:** これまでマクロの保存・削除はナビペインの「マクロ」ビューを開く必要があったが、ステータスバー右下のマクロ（⚡）プルダウンに「現在のスクリプトを保存」ボタンを追加し、保存済みマクロを右クリックして「編集（エディタに読込）」「削除」できるようにした
