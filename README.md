...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.28.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.28_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.28_delta_from_0.40.27.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.28] - 2026-04-02 : テーマカラー個別化・カスタマイズ画面改善

### Added
- **コンテキストメニューにクイックジャンプ追加:** ファイル一覧の背景・フォルダ・ファイル右クリック時のコンテキストメニューからクイックジャンプを起動可能に。ショートカットキー表示付き
- **ナビペイン・コントロールデッキ・ファイル一覧の個別テーマカラー:** ナビペイン（通常/ホバー/選択の背景色・テキスト色 6キー）、コントロールデッキ（ホバー/選択 2キー）、ファイル一覧テキスト色（通常/ホバー/選択 3キー）計11個の専用カラーキーを追加。テーマカスタマイズ画面に「ナビペイン」セクションを新設。GrepSearch 3キーを検索セクションに統合
- **テーマカスタマイズ画面の改善:** ウィンドウ幅を拡張し横スクロールバーを解消。各セクション内のカラー項目をサブグループ（背景・テキスト・ボタン等）に分類し、グループヘッダー付きで表示するよう改善
