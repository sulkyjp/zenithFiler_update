...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.1_delta_from_0.40.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.1] - 2026-03-26 : ウェルカムウィンドウ刷新 + ローカライズ即時反映 + リファクタリング

### Added
- **英語版EULA:** EULA表示をロケール対応（EULA_{locale}.md → EULA_en.md → EULA.md のフォールバック）

### Changed
- **ウェルカムウィンドウ刷新:** 初回起動時に言語選択ページを先頭に追加（システムロケール自動検出）。テーマ選択をカテゴリ別グループ表示+色プレビュー+おすすめセクション付きに改善。イントロ説明文を最新機能に更新
- **ローカライズ即時反映:** 言語変更時にナビペインヘッダー・ツールバーツールチップが即座に切り替わるように改善。LocaleChanged イベントで NavViewItem ラベル再取得・ツールチップ再構築を実行
- **リファクタリング:** EnsureTwoPaneMode() / ApplyColumnWidth() ヘルパー抽出による重複排除。PaneTerminalControl・MainWindow のタイマーメモリリーク修正

### Fixed
- **言語JSONパースエラー修正:** 6言語ファイル（de/es/ko-KR/pt/zh-CN/zh-TW）の未エスケープダブルクォートを修正。言語切替が正常に動作するように
