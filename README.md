...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.2_delta_from_0.40.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.2] - 2026-03-26 : 安定性修正 + UX改善

### Changed
- **言語変更後の再起動案内:** 設定で言語を変更した際に、再起動を促すバナーと再起動ボタンを表示
- **AIお気に入り整理のロック時案内:** ロック中にAI整理を実行しようとした際、設定ダイアログを閉じずにロックチェックボックスを点滅して案内
- **AIお気に入り整理で仮想フォルダにも説明文を生成:** 仮想フォルダ（グループ）の目的説明を自動生成するようプロンプトを改善
- **作者構成プリセットに左下ペイン幅自動調整を追加**
- **ナビペイン下段ペインの幅自動調整をデフォルト有効化:** 左下=プロパティ、右下=よくアクセスするフォルダ

### Fixed
- **起動時フリーズ修正:** 画面外退避中に DispatcherPriority.Loaded の await が永久ブロックする問題を修正
- **タブ切替アニメーションの起動時クラッシュ修正:** ビジュアルツリー未構築時の子要素アクセスにガードを追加
- **右ナビペイン下段の幅自動調整修正:** 幅自動調整時に ViewModel の値のみ更新され、実際のカラム幅が変わらない問題を修正
