...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.7.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.7_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.7_delta_from_0.38.6.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.7] - 2026-03-22 : お気に入り機能強化（カラータグ・バッジ・ABC順・最近追加）

### Added
- **お気に入りカラータグ:** お気に入りアイテムに7色（レッド・オレンジ・イエロー・グリーン・ブルー・パープル・グレー）のカラーラベルを割り当て可能。macOS Finder スタイルのカラードットがアイテム名の前に表示。右クリックメニューから設定・解除。`tag:red` 形式の検索プレフィックスでカラータグによる絞り込みに対応。ツリー・ピン留め・最近追加・ABC順・検索結果の全ビューで表示。設定は JSON に永続化
- **お気に入りアイテム数バッジ:** お気に入りツリーの仮想フォルダ（コンテナ）名の横に、含まれるリーフアイテム数をバッジ表示。子コンテナ内のアイテムも再帰的にカウント。お気に入り設定ダイアログから表示/非表示を切り替え可能
- **お気に入り参照回数表示:** お気に入りツリーの各アイテム名の横に履歴DBの参照回数を `(N)` 形式で表示。お気に入り設定ダイアログのトグルで表示/非表示を切り替え可能。検索結果リストにも対応
- **エクスプローラからお気に入りへのドラッグ＆ドロップ登録:** Windows エクスプローラからファイルやフォルダをお気に入りツリーにドロップしてお気に入りに登録可能。仮想フォルダにドロップすると子として追加、空白領域にドロップするとルートに追加。重複チェック・ロック中はドロップ拒否・追加件数の通知に対応
- **お気に入り「最近追加」セクション:** お気に入りビューの「よく使う」セクションの下に、過去7日以内に登録されたアイテムを最大5件表示。相対日付（今日・昨日・N日前）付きで表示。ヘッダーの時計アイコンまたは設定ダイアログから表示/非表示を切り替え可能
- **お気に入りABC順ソート:** お気に入りビューのヘッダーにツリー/ABC順の切替トグルを追加。ABC順モードではコンテナを除く全アイテムをフラットにアルファベット順（日本語かな対応）で表示。パス情報付きのリスト表示でダブルクリック・中クリック・コンテキストメニュー・ドラッグ操作に対応。設定は次回起動時に復元
