...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.8.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.8_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.8_delta_from_0.38.7.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.8] - 2026-03-22 : ターミナル設定独立化 + ナビペイン幅自動設定 + プロパティ改善

### Added
- **ペインターミナル設定ポップアップ:** A/Bペインのターミナルタブにアクションバーから設定ポップアップを追加。フォントファミリー・フォントサイズ・背景色・文字色・テーマ連動を設定可能。「全ターミナルで共通」チェックでグローバル設定とペイン別設定を切り替え可能。共通モードでは全ターミナルに設定を即時反映、個別モードではペインごとに独立設定を保持
- **ナビペイン幅自動設定:** 下段ヘッダーに幅自動設定ボタン（ArrowLeftRight アイコン）を追加。有効にすると下段コンテンツの必要幅を測定してナビペイン幅を自動調整。ターミナルビューは対象外。設定はアイテムごとに永続化
- **ターミナル cd ドロップダウン統一:** 全ターミナル（A/Bペイン・ナビペイン）のアクションバーに「A cd」「B cd」ボタンを配置。クリックで対応ペインのフォルダタブ一覧をドロップダウン表示し、選択フォルダへ即座に cd 実行

### Changed
- **プロパティビュー表示改善:** ファイル名行を廃止しフルパス表示に統合。フルパス枠は6行分の固定高さを確保し、長いパスはスリムスクロールバーで全文確認可能。種類名の TextWrapping を TextTrimming に変更し、高さの安定化を実現
- **右ナビペイン幅の永続化修正:** 右ナビペインのスプリッタードラッグ後の幅がアプリ終了時に正しく保存されない問題を修正
