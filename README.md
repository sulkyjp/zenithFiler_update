...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.8.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.8.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.8.1_delta_from_1.8.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.8.1] - 2026-07-13 : AIプロバイダ「Claude Code (CLI)」追加とキーレスプロバイダの未実行バグ修正、配布パッケージの内蔵ターミナル欠落修正

### Added
- **AI プロバイダに「Claude Code (CLI)」を追加:** ローカルにインストール済みの Claude Code をヘッドレス実行する 6 番目のプロバイダ。サブスクリプション（Pro/Max）でログイン済みなら **API キーの設定なし**で全 AI 機能（リネーム提案・テーマ生成・フォルダ整理・マクロの `zen.ai()` 等）を利用できる。設定画面に専用タブ（インストール検出・実行ファイルパス上書き・接続テスト・モデル選択 default/sonnet/opus/haiku/fable）を新設。CLI 未インストール・未ログイン時は専用のガイダンス文言を表示する。制約: このプロバイダでは最大トークン数・Temperature 設定は適用されず、CLI 起動のため応答に数秒のオーバーヘッドがある。なお本バージョンの設定ファイルを旧バージョンで開くと AI プロバイダ選択が既定値に戻る場合がある

### Fixed
- **Ollama をアクティブプロバイダにしていると、ほとんどの AI 機能が「API キー未設定」扱いで実行できなかった問題を修正:** AI 機能の入口チェック（テーマ生成・AI マニュアル・リネーム提案・統計分析・Stickman 生成・バックアップサマリー等）が API キーの有無を直接判定しており、キー不要のプロバイダでは常に「未設定」と誤判定されていた。キー不要プロバイダを考慮する共通判定に統一した（新規追加の Claude Code (CLI) も同様に動作する）
- **Ollama のエンドポイント設定がアプリ終了時に失われることがあった問題を修正:** 終了時の設定引き継ぎ処理に Ollama エンドポイントが含まれておらず、セッション中に保存された値が終了時の書き戻しで既定値に戻ることがあった
- **配布パッケージから内蔵ターミナルの中継プロセス一式と内容検索エンジンが欠落していた問題を修正:** 自動リリースビルド（クリーン環境）では、ビルド設定の判定タイミングの問題により、内蔵ターミナルが必要とする中継プロセス一式（conpty_host ほか）と、ファイル内容検索が使用する検索エンジン（ripgrep）が配布 zip に含まれていなかった。このため配布版ではターミナルタブが「起動失敗」となり、内容検索は結果が常に空になっていた（開発環境のビルドでは再現しないため長期間気づけなかった）。必須コンポーネントをビルド時に確実に同梱し、欠落時はビルド自体を失敗させるように修正。あわせて中継プロセスを .NET ランタイム非依存の自己完結型バイナリに変更し、.NET ランタイム未導入の環境やランタイム同梱フォルダ内でも確実に起動できるようにした。さらにリリース工程にも同梱チェックを追加し、再発時は配布前に検出されるようにした
