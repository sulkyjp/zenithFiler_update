...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.0_delta_from_0.39.3.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.0] - 2026-03-25 : UI洗練 + Pro制限調整 + バグ修正

### Added
- **テーマスケジュール切替（Pro限定）:** 時間帯に応じて昼テーマと夜テーマを自動切替する新モード。テーマ設定画面で「スケジュール」を選択し、昼/夜のテーマと切替時刻を指定。1分間隔で時刻をチェックし自動適用
- **Pro専用テーマ「ProExclusive」:** ディープネイビー×ゴールドアクセントのプレミアムテーマ。Pro登録時に自動付与
- **2画面上下分割:** ステータスバーに上下分割ボタン（Rows2アイコン）を追加。ファイル名が長い場合に横幅を確保できる。分割比率は保存・復元対応
- **ショートカットキー拡充（25→36アクション）:** タブ操作（Ctrl+T/W/L/Shift+D）、ファイル操作（Ctrl+Shift+E: エクスプローラーで開く、Ctrl+Shift+C: パスコピー、Ctrl+D: お気に入り追加、Alt+Enter: プロパティ）、上下2画面モード切替を設定画面から変更可能に

### Changed
- **テーマ変更の無料回数調整:** Free版のテーマ変更回数を5回/日から3回/日に変更
- **レイアウトプリセット保存の無料枠追加:** Free版でも1回までレイアウトプリセットの保存が可能に
- **列自動非表示の動的制御:** 名前列に最低250pxを確保する動的閾値方式に変更。通常モードは場所→種類→サイズ→更新日時、検索モードは場所→サイズ→更新日時→Match→名前の順に非表示
- **ペイン境界線のスリム化:** GridSplitterの見た目を1pxに変更し、マウス判定領域は7pxを維持。ホバー時にアクセントカラーで発光するエフェクトを追加

### Fixed
- **ターミナルタブのキーボード入力修正:** MainWindowのPreviewKeyDown/KeyDownがターミナルタブのキーイベントを横取りしていた問題を修正。ターミナルにフォーカスがある場合はキーイベントをそのまま通すように変更
- **ペインモード切替ボタンのハイライト修正:** 左右分割ボタンが上下分割中でもハイライトされる問題を修正。3つのボタン（1画面/左右/上下）が排他的にハイライトされるように変更
- **上下分割モードの矢印キー修正:** 分割モードに関わらず←→でペイン切替するよう統一。↑↓はファイル操作用に確保
- **分割方向の復元修正:** 検索後のペイン数復元、反対ペインでのパス/検索結果を開く操作、ペイン数トグルで分割方向（上下/左右）が保持されない問題を修正
- **BOXリンク取得不正の修正:** フォルダの共有リンクコピー時に親フォルダのリンクが取得される問題を修正。フォルダ選択時はフォルダ自体の共有リンクを取得するように変更
- **ナビペイン幅調整の修正:** 高DPI環境でGridSplitterが掴めない問題を修正。スプリッター幅を1px→5pxに拡大、背景色をテーマ連動（BorderBrush）に変更、SnapsToDevicePixels有効化
- **リカバリ後のお気に入り復元:** リカバリ（settings.json復元）後にお気に入りがメモリに反映されない問題を修正。復元直後にお気に入りをリロードし、再起動なしで即座に反映
- **Pro購入後の特別演出:** ライセンス登録後にGlowBarお祝い演出、「すべての機能が解放されました」通知、ステータスバーにPROバッジ表示、Pro専用テーマ「ProExclusive」（ネイビー×ゴールド）を付与
- **アップデートチェックのrate limit対策:** GitHub API rate limit (403) でアップデートチェックが連続失敗する問題を修正。前回チェックから30分以内はスキップ、rate limit時は専用エラーハンドリング
