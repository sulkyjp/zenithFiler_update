...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.7.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.7.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.7.0_delta_from_1.6.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.7.0] - 2026-07-09 : テーマの「テーマ作成」「ストア」カテゴリ追加とテーマストアの不具合修正

### Added
- **テーマ一覧に「テーマ作成」「ストア」カテゴリを追加:** 自分で作成・カスタマイズしたテーマは「テーマ作成」、ストアからダウンロードしたテーマは「ストア」として、既存5カテゴリ（Standard/Lifestyle/Professional/Premium/RetroTech）とは別のグループにまとめて表示するようにした。これにより公式テーマとユーザー由来のテーマが混在せず、テーマ一覧の見通しが良くなる。ダウンロードしたテーマには由来情報を記録し、正しく「ストア」カテゴリに振り分ける。テーマの作成・削除は、テーマ一覧上部のカテゴリ見出しへ即座に反映される

### Fixed
- **テーマをストアにアップロードしても一覧に反映されず、説明も表示されない問題を修正:** アップロード直後のストア再取得が、コミュニティ（ユーザー投稿）テーマを合流させずに公式テーマ一覧だけで再構築していたため、投稿したテーマが一覧から消えていた。またストア一覧とマイアップロードの再読み込みが並行実行され、説明・色の補完が競合して空欄になることがあった。両処理を直列化し、コミュニティテーマも必ず合流するようにして解消した
- **ストアのテーマ評価がカードに反映されず常に0と表示される問題を修正:** 静的な一覧データの評価値（ほぼ0）だけを表示しており、最新の評価集計を取得していなかった。テーマ一覧と同じ集計を取得してカードへ反映するようにした。あわせてカードの星は自分の点ではなく常に全体平均を表すように統一した
- **長いテーマ名がカードからはみ出し、バッジや隣の要素と重なる問題を修正:** テーマ名の表示幅が制限されておらず省略（…）が働いていなかった。テーマ一覧・ストアの両カードで、長い名前を「…」で省略し、「インストール済み」バッジと重ならないようにした
