...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.3.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.3.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.3.1_delta_from_1.3.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.3.1] - 2026-06-18 : ファイル選択・フォルダ移動時の一時フリーズを修正、検索/閲覧履歴ビューの表示を最適化

### Changed
- **検索履歴・閲覧履歴ビューの表示処理を最適化 (#72):** ビューを開く際に一覧をまとめて反映するようにし（従来は項目を1件ずつ追加していた）、リストの仮想化を有効化した。これにより履歴ビューを開いた瞬間のもたつきを解消し、スクロールも滑らかになった

### Fixed
- **ファイル選択・フォルダ移動時に一瞬固まることがある問題を修正:** 選択したファイルの作成日時・画像情報の取得をバックグラウンドに移し、操作中に画面が固まらないようにした。プロパティ・プレビューの細かな情報は一拍遅れて表示されるが、操作はブロックされない
