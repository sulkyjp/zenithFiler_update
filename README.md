...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.2.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.2.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.2.0_delta_from_1.1.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.2.0] - 2026-06-11 : 信頼性・堅牢性の全面強化（異常終了の検知基盤／高速操作時の安定化／終了処理の保全）

### Added
- **異常終了の検知・診断基盤を追加:** 強制終了・電源断・ネイティブクラッシュなど、通常のエラーハンドラを素通りする異常終了を次回起動時に検知し、前回セッションのログ末尾を診断ログへ転記するようにした（クリーンシャットダウン・センチネル方式。検査・作成はバックグラウンド実行のため起動速度への影響なし）

### Fixed
- **クラッシュ時のエラーダイアログとログ保全を堅牢化:** 致命的エラーがバックグラウンドスレッドで発生した場合、エラーダイアログの表示処理自体が二次エラーを起こし、診断情報が失われることがあった。ダイアログ表示を画面側スレッドへ正しく委譲し、プロセス終了前に非同期ログのバッファを確実に書き出すようにした
- **実績共有の接続先が見つからない場合の連続エラーを解消:** 共有先（USBメモリ・同期フォルダ等）のドライブが接続されていない場合、起動のたびにエラーが記録され続けていた。接続先の事前チェックを追加し、連続して失敗した場合はそのセッション中の同期を自動的に一時停止するようにした（設定自体は変更されず、次回起動時に自動で再試行される）
- **使用回数照会の応答性と日付切替時の堅牢性を改善:** 起動直後に内部データの照会で画面が固まりうる経路に上限時間を設け、照会結果はバックグラウンドで事前読み込みするようにした（操作が常に即応）。日次キャッシュの日付切替判定もマルチスレッド安全に強化した
- **バックグラウンド処理の例外による異常終了経路を封じ込め:** インデックス定期更新の進捗通知がバックグラウンドスレッドから画面の通知領域に直接書き込まれることがあり、過去に異常終了の実績があった経路を画面側スレッドへの委譲で根絶した。また、設定画面の一部の非同期処理（管理メニュー判定・診断情報更新・ペイン別テーマ適用・更新ダウンロード）で、例外が発生するとアプリ全体が落ちる構造になっていたものを、例外を確実に記録して握りつぶす安全な構造に統一した
- **アプリ終了処理の堅牢化（設定消失防止）:** 終了時のクリーンアップ処理のいずれかが失敗すると、後続の終了処理がスキップされて設定が保存されない可能性があった。各終了段階（サービス停止・購読解除・設定保存）を独立してガードし、途中で問題が起きても設定保存とリソース解放に必ず到達するようにした。終了時に動いている定期処理のタイマーも確実に停止するようにした
- **ファイル右クリックメニューのコールバック例外を保護:** メニュークローズ時の後処理で例外が発生した場合に異常終了につながる経路があったため、確実にガードするようにした
- **ペイン構成・ナビペインの高速切替時の安定性を強化:** ペインの開閉やナビペインの切替を素早く連続で行った際（特にファイルアクセスの遅い環境）に、内部処理の競合で異常終了につながる可能性があった。ターミナル（WebView2）の初期化中にペインが閉じられた場合の中断ガード、切替連打時に古い復帰処理を破棄する世代管理、タブキャッシュ破棄・レイアウト再構築の例外からの自己回復を追加し、連打しても壊れないようにした（切替速度は従来どおり）
