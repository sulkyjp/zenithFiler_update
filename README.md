...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.23.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.23_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.23_delta_from_0.40.22.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.23] - 2026-03-31 : タブ管理の大幅強化（ショートカット・管理ビュー・設定・AI連携）

### Added
- **タブ一覧・タブ切替のショートカットキー対応:** Ctrl+Shift+A でフォーカスペインのタブ一覧ポップアップ表示、Ctrl+Tab / Ctrl+Shift+Tab で次/前のタブ切替。Aペイン・Bペイン個別のタブ一覧表示（Ctrl+Shift+Alt+1/2）にも対応。すべてショートカット設定画面でカスタマイズ可能
- **タブ一覧ポップアップのキーボード操作:** タブ一覧表示時に ↑/↓ でタブ選択、Enter で切替、Delete / Ctrl+W でタブを閉じる、Escape でポップアップを閉じてファイルリストにフォーカス復帰
- **タブバー表示設定:** 設定画面に「タブ」カテゴリを新設。タブ幅モード（固定幅/可変幅）、固定幅サイズ（100〜250px）、折り返しモード（1行/複数行）、A/Bペインのタブ行高さ同期を設定可能
- **ナビペインのタブ管理ビュー:** A/Bペインのタブを横並びで一覧表示。クリックでタブ切替、×ボタンでタブ閉じ、右クリックでロック切替・ペイン間移動。ドラッグ＆ドロップでタブの並替・ペイン間移動に対応。AI推薦ドット（ロック推薦/クローズ推薦）も表示
- **タブアクティビティの可視化:** タブ管理ビューの各タブにアクティビティスコアのプログレスバーを表示。使用頻度の低いタブはグレーアウト
- **コンテキスト連動ハイライト:** 現在のフォルダとパスの親子関係があるタブをアクセントカラーのドットで自動ハイライト。フォルダ移動やペイン切替にリアルタイム追従
- **ワーキングセット連携:** タブ管理ビューのヘッダーからワーキングセットとして保存可能。保存済みセットと現在のタブ構成が一致する場合はセット名を表示
- **タブサマリー表示:** タブ管理ビュー下部にA/Bペインのタブ数、総滞在時間、よく使うフォルダTop3のミニ統計を表示

### Changed
- **タブ一覧の選択動作を改善:** ↑/↓キーでの移動中はタブが切り替わらず、Enter 押下時に初めてタブが切り替わるように変更
- **別ペインのタブ一覧表示を改善:** Ctrl+Shift+Alt+1/2 で別ペインのタブ一覧を表示する際、先に対象ペインにフォーカスを移動してからタブ一覧を表示するように変更
- **タブ管理ビューのフォーカスペイン表示:** A/Bペインのヘッダーにアイコン（PanelLeft/PanelRight）を追加し、アクティブペイン側を太字+アクセントカラーで強調。ペイン領域クリックでフォーカス移動
- **タブ管理ビューのツールチップ強化:** 各タブのホバーにフルパス・ロック状態・AI推薦・コンテキスト連動の意味を複合ツールチップで表示。ドットサイズを拡大しホバーしやすく
- **新規タブをパンくずモードで開く:** パス付きの新規タブ追加時、アドレスバーをフルパスモードではなくパンくずモードで表示するように変更

### Fixed
- **タブ行高さ同期の無限ループを修正:** 折り返しモード時のタブ行高さ同期処理で SizeChanged イベントが連鎖し UI がフリーズする問題を修正
- **タブ管理ビューのコンテキストメニュー修正:** Separator に MenuItem 用スタイルが適用されクラッシュする問題を修正
- **タブ管理ビューのホバー背景色修正:** ListBoxItem のヒットテスト領域不足でホバー背景色が表示されない問題を修正
