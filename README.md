...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.41.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.41.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.41.2_delta_from_0.41.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.41.2] - 2026-04-06 : 検索バー刷新・XPランキング・タブ管理改善

### Added
- **XP ランキングシステム:** チャレンジ XP をサーバーにアップロードし、ユーザー間のランキングを表示。レベルアップ時に自動送信（24時間間隔）。アップロード拒否・実名・匿名の3択。ランキングダイアログでトップ100を表示
- **検索バーUIの刷新:** アイコントグル式のモード切替を4セグメントコントロール（Search/Grep/Index/Filter）に置き換え。直感的なモード切替と各モードのツールチップを実装
- **抽出（Filter）モード:** 現在表示中のフォルダ内をリアルタイムでインクリメンタル絞り込みする新モードを追加
- **検索モードのテーマカスタマイズ:** 各モードの選択時背景色・テキスト色、通常検索・Filter検索の背景色・アイコン色・フォーカス枠色をテーマ別に設定可能に
- **検索アイコン色の反映:** 各検索モード（通常/Grep/Index/Filter）のアイコン色がセグメントコントロール上のアイコンに反映されるよう修正

### Fixed
- **タブ管理ビューのペインフォーカス切替:** ナビペインのタブ管理ビューでAペイン/Bペイン領域（タブ・空きスペース）をクリックした際にフォーカスが切り替わらなかった問題を修正
- **タブバー非表示オプションの起動時反映:** タブ管理ビュー表示時にタブバーを非表示にするオプションが起動直後に反映されなかった問題を修正

### Changed
- **検索モードのペイン共通化:** 検索モードをタブ単位からペイン単位に変更。タブを切り替えてもモードが維持され、次回起動時も復元
- **Filterモード時のオプション非活性:** Filterモード選択時はサイズ・日付・プリセットのフィルタボタンを非活性表示
