...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.6.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.6.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.6.0_delta_from_1.5.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.6.0] - 2026-07-07 : ペイン間ファイルコピーの高速化・進捗表示強化、UIフリーズ対策の仕上げ

### Fixed
- **起動時に常時発生していたマスコット行動データの同期読み込みを解消:** ステータスバーマスコット（Stickman）の表示設定（既定オフ）の値に関わらず、`stickman/`以下の行動定義JSON（基本+報酬+ストア分 計170本超）を`MainWindow.xaml`の`InitializeComponent`中にUIスレッドで同期読み込みしていた問題を修正。実際にマスコットが有効化されるタイミングまで読込を遅延させ、BGスレッドで実行してから登録するように変更した。マスコット非表示ユーザーはこの読込コストが完全になくなる
- **Stickmanのフレーム描画ループにクラッシュガードを追加:** 毎フレーム発火する描画コールバックに例外処理がなく、AI生成/ストアDL由来の不正な行動JSONが混入すると毎フレーム例外でアプリごと落ちる恐れがあった問題を修正。不正な行動1件は隔離して継続し、想定外の連続エラー時のみマスコットを自動無効化するようにした
- **設定画面のStickmanプレビューが閉じた後も描画処理が回り続けるリークを修正:** プレビューコントロールがアンロード時に描画フック（CompositionTarget.Rendering）を解除しておらず、ControlDeckの開閉を繰り返すたびに描画処理が積み重なっていた問題を修正
- **ファイルのコピー/切り取り時に、監視ソフト環境でアプリ全体が数秒〜数十秒固まることがある問題を修正:** コピー/切り取り/パスのコピーなど、クリップボードへの書き込みがすべてUIスレッド上で同期実行されていたため、監視ソフト（EDR/DLP等）がクリップボード書き込みへ割り込む環境では、その完了までアプリが完全に無反応になっていた。書き込みを専用のバックグラウンドスレッドへ移し、完了時に通知を出す方式に変更。クリップボードが他プロセスに使用中だった場合のリトライ待機（最大約350ms）もUIスレッドから排除した
- **ファイル選択時のプレビュー/ホバーサムネイルによる引っかかりを解消:** 動画・SVGプレビューやホバーサムネイルの取得前処理で、ファイルの存在確認・属性取得がUIスレッド上で同期実行されていた。クラウドストレージ（Box/OneDrive）のプレースホルダーと監視ソフトが重なる環境では選択のたびに数秒ブロックし得たため、サムネイル取得用のワーカースレッド側へ移動した
- **貼り付け・ドロップ開始時のUIフリーズを解消:** 貼り付け/ドロップ開始前に行う転送元・転送先の存在確認と上書き競合チェック（ファイルサイズ・更新日時の取得）がUIスレッド上で同期実行されていたため、クラウドパスや監視ソフト環境で数秒固まることがあった。バックグラウンドスレッドで実行するよう変更し、あわせて貼り付け時のクリップボード読み取り回数も削減した（監視ソフトの割り込み機会を減らすため）
- **アイコンビュー切替直後の引っかかりを解消:** 一覧をアイコンビューへ切り替えた際のサムネイル読み込み開始処理だけが、フォルダ読み込み時の主経路（バックグラウンド実行）と異なりUIスレッド上でシェルAPIを連続呼び出ししていた非対称を修正
- **コピー/移動中のステータス表示が壊れていた問題を修正:** ステータス文言の書式に対して実際に渡す引数の数がずれており、コピー中は件数が表示されず、コピー/移動ともに対象ファイル名が表示されていなかった

### Changed
- **Stickmanの本体（ステータスバー）とプレビューで二重実装されていた演出描画を共通化:** 本の積み上げ・机/王座・創造の光球・後光・骨投げ・フォロワー（従者）・小道具の描画ロジックを`StickmanEffectsRenderer`に集約し、今後の演出追加・修正で両ファイルを手動同期する必要がなくなった。あわせてプレビューの描画間引き・ブラシキャッシュ化・毎フレームのオブジェクト生成削減によりCPU/GC負荷を軽減
- **ペイン間ファイルコピーのエンジンをダブルバッファ・パイプライン化:** 4MB超のファイルは「現チャンクの書込」と「次チャンクの読込」を並行実行するように変更し、読込→書込の直列待ちを解消してスループットを底上げした
- **ファイルコピーの並列転送数を引き上げ（上限3→8）:** コア数に応じた並列度の上限を、robocopy の `/MT` 既定値相当まで引き上げた。ストレージ特性によっては効果が環境依存のため、実機での挙動確認を継続する

### Added
- **UIフリーズの自動検出ログ（[UI-HANG]）を追加:** UIスレッドが500ms以上応答しない状態をバックグラウンドから常時監視し、回復後に実測のブロック時間をログへ記録するようにした。環境依存（監視ソフト・クラウドストレージ等）のフリーズ報告時に「いつ・何秒固まったか」をログから特定できる。監視コストは2秒に1回の軽量プローブのみ
- **ファイルコピー中の進捗表示に件数・残り時間・現在のファイル名を追加:** 「N / Total 件をコピー中... — 転送速度 — 残り時間 — ファイル名」を1行で表示するようにした。残り時間は開始2秒後から表示し、開始直後の速度乱高下による表示ブレを抑制している
