...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.22.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.22_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.22_delta_from_0.40.21.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.22] - 2026-03-30 : 設定画面 UI 全面リファイン + バグ修正

### Added
- **レベルバナー・二つ名バナーのクリックでチャレンジページに遷移:** ヘッダーのレベルバナーと二つ名バナーをクリックするとチャレンジページが開くよう追加。既存のチャレンジアイコンと同じ動作

### Changed
- **設定画面のボタンスタイル・ホバー演出を全ページで共通化:** インラインスタイルを排除し共通スタイル（TextButton/AccentButton）に統一。OptionRadioButton・テーマモード選択カード・言語設定・Debug ボタン等のホバー演出を統一。壊れていた言語選択のホバーも修正。新テーマカラー `ButtonIdleColor` を追加し、未選択ボタンにカード背景と区別できる薄い背景色を設定。言語設定をカードで囲み他ページと統一、再起動バナーを言語リストから独立したレイアウトに変更
- **ライセンスページをダッシュボード風にリデザイン:** 基本設定のデザイン言語（白カード・細枠線・統一余白）を継承。ヒーローカード→アクション→テレメトリーの縦ストリーム構成に。使用状況ゲージを3pxスリム化、数値をConsolas表記に。各グループをカードで囲みモダン・フラットな質感に統一
- **テーマページにカテゴリアンカーを常時表示:** ページヘッダ直下に5カテゴリ（スタンダード/ライフスタイル/プロフェッショナル/プレミアム/レトロ&テック）のジャンプリンクを固定配置。カテゴリ間の余白を縮小しコンパクトに
- **スケジュールモードのテーマ選択をカード直接選択に変更:** ComboBox プルダウンを廃止し、ペイン個別適用と同じ RadioButton 方式に統一。日中/夜間スロットを選択してギャラリーからテーマカードをクリックして適用。カードサイズを他の3モードと統一（Width=320）
- **言語設定に補足情報を追加:** AI 翻訳のニュアンス差異に関する注記と、lang フォルダへの言語ファイル追加で自動反映される旨を記載
- **ショートカットキー設定のキーバインド領域に背景色を設定:** ButtonIdleBrush を適用し、クリック可能な領域であることを視覚的に明示
- **自動アップデートのバージョンチェックを Cloudflare Worker 経由に変更:** GitHub API の rate limit（60回/時間/IP）を回避。Worker が5分間キャッシュし全ユーザーの API 消費を集約。Worker 失敗時は GitHub API に自動フォールバック

### Fixed
- **設定画面の左メニューでホバー時に背景色が変化しない問題を修正:** サイドバー背景と同色のブラシを使用していたため、ホバーが視認できなかった
- **ペイン個別適用モードで初回クリック時にテーマが適用されない問題を修正:** モード切替時に SelectedPaneTarget が None のままだったため、テーマ選択が無視されていた。切替時に自動で LeftNav を選択するよう修正
- **言語設定のホバーが動作しない問題を修正:** 未定義の HoverBrush を参照していたため、ホバー時の背景色変化が発生しなかった
- **テーマカラー一括追加ツール（add_theme_color.py）のカンマ欠落バグを修正:** 新規行挿入時に前行・新行の末尾カンマが付与されず、JSON パースエラーが発生していた
