...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.4.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.4.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.4.1_delta_from_1.4.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.4.1] - 2026-06-25 : クラウド/ネットワークフォルダでのフリーズと常駐モードの終了処理を改善

### Fixed
- **クラウド同期・ネットワーク共有フォルダを開いた際に一瞬固まることがある問題を修正:** フォルダ移動のたびに行うフォルダ監視の準備を、画面操作をブロックしないようバックグラウンドに移した。Box / OneDrive やネットワーク共有など応答が遅い場所を続けて開いても、画面が固まりにくくなった
- **常駐モードでの終了処理を改善:** Windows のサインアウト／シャットダウン時にトレイ常駐から確実にクリーン終了するようにし、次回起動時に「前回が正常終了しなかった」と誤って記録されることがあった問題を解消した（終了時の内部エラーログも抑制）
