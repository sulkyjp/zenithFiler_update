...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.0.4.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.0.4_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.0.4_delta_from_1.0.3.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.0.4] - 2026-06-07 : テーマの全面品質刷新（一覧ミニプレビュー・行罫線の視認性・Premium外周線）とドラッグ＆ドロップの操作性向上

### Changed
- **ドラッグ＆ドロップ中に投入先フォルダを強調表示:** ファイルをドラッグしてフォルダに重ねると、その行が選択時と同じハイライトで強調されるようにした。どのフォルダに入るのかが一目で分かる（詳細／アイコン両ビュー対応。エクスプローラー等からの外部ドロップや右ドラッグでも有効）
- **テーマ一覧の各カードをそのテーマ色のミニプレビューに刷新:** これまでテーマカードは現在のテーマ色で一律に描画され、各テーマの特色は小さな丸スウォッチ4個でしか分からなかった。カードの背景・左サイドバー帯・文字・★・ペインバッジを「そのテーマ自身の色」で描き、サンプルの選択行（アクセント色）と通常行を添えた“アプリの縮小プレビュー”に作り替えた。一覧を眺めるだけで各テーマの雰囲気・コントラスト・明るさが一目で比較できる
- **ファイル一覧の行罫線をテーマ独立で見やすく:** これまで行の区切り線は共有の枠線色を使っており、明るいテーマでは背景に埋もれて行が追いづらかった。一覧の行罫線専用の色を新設し、どのテーマでも背景と明確なコントラストを保つようにした（ファイラとしての視認性を最優先）
- **Premium テーマにウィンドウ外周のアクセント線を追加:** Premium カテゴリのテーマに限り、ウィンドウ外周へテーマアクセント色（深いトーン）の細い縁取りを表示し、特別感を演出できるようにした（テーマ側で設定。非 Premium テーマでは表示されない）

### Fixed
- **フォルダにドロップした後、そのフォルダが選択されたまま維持されるよう修正:** ファイルをサブフォルダにコピー/移動した後、選択とフォーカスが一覧の先頭アイテムに飛んでしまい「どこに入れたか」が分からなくなっていた。投入先フォルダが選択・フォーカスされた状態を維持するようにした（現在のフォルダへ直接投入した場合は従来どおり投入したファイルを選択する）
- **操作後に対象が現在の一覧に無い場合、選択が先頭に飛ばないよう修正:** ファイル操作（移動・ショートカット作成など）の後、対象のアイテムが現在表示中のフォルダに存在しないとき、選択とフォーカスが一覧の先頭アイテムに移動してしまうことがあった。このような場合は現在の選択状態を維持し、意図しない先頭ジャンプを排除した
