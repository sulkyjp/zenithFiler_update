...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.41.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.41.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.41.1_delta_from_0.41.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.41.1] - 2026-04-05 : テーマUI統一・ストア機能強化・安定性改善

### Added
- **選択状態のテキスト色を個別制御:** `OptionSelectedTextColor` を新設。設定画面の選択ボタン・タブ管理のフォーカスタブで背景色と同系色のテキストが読めなくなる問題を解消
- **ダイアログボタンのテキスト色を個別制御:** `DialogButtonTextColor`・`DialogPrimaryButtonTextColor` を新設。ダイアログのボタン文字がテーマによって読めなくなる問題を解消
- **テーマカスタマイズウィンドウに検索機能:** ヘッダー右側に検索ボックスを追加。キー名・説明で色設定項目を絞り込み、該当セクションに自動遷移
- **マイアップロードの折りたたみ:** テーマストアのマイアップロードセクションをシェブロンクリックで折りたたみ/展開可能に。状態は次回起動時にも保持
- **テーマストアのページネーション:** テーマ一覧を50件ずつページ表示し、大量テーマ時の負荷を軽減

### Changed
- **管理者識別をメールからストアIDに統一:** Worker の isAdmin フラグを利用し、メールアドレスに依存しない管理者判定に変更
- **テーマストアからデフォルトテーマを非表示:** 自動更新されるビルトインテーマをストア一覧から除外し、ユーザーアップロードテーマのみ表示
- **テーマストアの管理者・作成者削除:** ストア一覧の右クリックメニューからテーマを削除可能に。管理者は全テーマ、作成者は自分のテーマを削除できる
- **AIテーマ生成・カスタマイズの品質方針強化:** 生成プロンプトに「モダン・高い視認性・全体調和・洗練されたデザイン」の方針を追加。見づらい・調和の取れていないテーマの生成を抑制

### Fixed
- **Box Drive 連携の安定性向上:** async void メソッドを async Task に変更し、未処理例外によるクラッシュリスクを解消
- **クラウドメニューキャッシュの効率化:** キャッシュ溢れ時の最古エントリ検索を O(n log n) から O(n) に改善
- **パンくずリストのテキスト色:** ハードコードされた白色をテーマの `OnePointDarkTextColor` に変更し、テーマに応じた適切な色で表示
- **お気に入り仮想フォルダヘッダーのテキスト色:** グループヘッダー内のテキスト・アイコンのハードコード色を `OnePointDarkTextColor` に統一し、全テーマで視認性を確保
- **アドレスバーアイコンの個別テーマ制御:** `AddressBarEditIconColor`・`AddressBarFavoriteIconColor`・`AddressBarCopyIconColor` を新設。デフォルトは `OnePointDarkTextColor` と同値で、デザイナーが個別にカスタマイズ可能
- **ツールチップのテーマ統一:** 全アプリのツールチップをコンテキストメニューと同じテーマスタイル（背景色・角丸・影）に統一
- **タブ管理ビューの選択+ホバー対応:** 選択タブのホバー時に `OptionSelectedHoverColor` を適用し、テキスト色も `OptionSelectedTextColor` を維持
- **ハードコード白色の一掃:** ボタン・バッジ・チェックマーク・ステップ番号等のハードコード `Foreground="White"` 30件を `DialogPrimaryButtonTextBrush` / `OnePointDarkTextBrush` に置換し、全テーマで視認性を確保
- **タブポップアップの選択+ホバー対応:** FilePaneControl のタブポップアップで選択+ホバー時に `OptionSelectedHoverColor` を適用
- **ComboBox のテーマ統一:** 全プルダウンをモダンなテーマ対応スタイルに刷新。ドロップダウン背景・選択色・ホバー色をコンテキストメニューと統一。Editable ComboBox にも対応
- **AccentButton のテキスト視認性改善:** 文字列 Content のボタンで `DialogPrimaryButtonTextColor` が適用されない問題を修正（更新ボタン・ライセンス登録等）
