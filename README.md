...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.6.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.6.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.6.1_delta_from_1.6.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->
...
## Latest Changes — [1.6.1] - 2026-07-08 : コンテキストメニュー高速化・リネームダイアログのフォーカス奪取根絶

### Added
- **リネームダイアログにリアルタイム名前バリデーションを追加:** 入力停止後0.3秒で名前を検証し、使用できない文字（`\ / : * ? " < > |`）や予約名（`CON`・`PRN`・`NUL`等）を含む場合は警告表示+OKボタンを無効化。同じフォルダ内に同名のファイル/フォルダーが既に存在する場合も警告表示する（この場合はOK可）
