...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.14.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.14_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.14_delta_from_0.40.13.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.14] - 2026-03-28 : チャレンジレベルシステム + RPG 演出強化

### Added
- **グリーティングのチャレンジ情報反映オプション:** AI ウェルカムメッセージに二つ名・バックストーリーをコンテキストとして渡すトグルを追加。AI グリーティング ON 時のみ有効。二つ名自体はメッセージに含めず、トーン判断の材料として使用
- **チャレンジ区分ごとのヒントテキスト:** 各区分タイトルの下に、何をすればカウントが進むかの説明を表示。全11区分・10言語対応
- **チャレンジレベル表示:** 実績件数プログレスバーを廃止し、XP ベースのレベル表示に置換。1レベル = 100 XP、盾アイコン + プログレスバーで RPG 風に表示
- **レベルバナーのヘッダバー表示:** 二つ名バナーの左側に「Lv.N」バナーを追加。二つ名とは独立したトグルで表示/非表示を切替可能
- **レベルアップ演出:** レベルアップ時にヘッダバナーがポップアニメーション + トースト通知
- **バナー色のテーマ個別指定:** PRO/レベル/二つ名バナーの色をテーマごとに個別指定可能に（LevelBadgeColor / TitleBadgeColor）。デフォルトは AccentColor と同値
- **Debug: 二つ名・バックストーリー再生成ボタン:** デバッグメニューから個別に AI 再生成が可能

### Changed
- **XP / レベルのリアルタイム更新:** チャレンジカウンター変化時に即座に XP・レベル・バナーを再計算（アプリ再起動不要）
- **検索チャレンジの対象拡大:** インデックス検索のみだった検索チャレンジを、通常検索・Grep 検索も含む全検索種別に拡大。トラッキングを検索実行成功後に移動
- **ナビゲーションチャレンジの Tier 回数を引き上げ:** 50〜5,000 → 100〜30,000。日常的に累積する操作のため、中長期の使い込みを評価する設計に変更
