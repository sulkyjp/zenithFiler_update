...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.29.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.29_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.29_delta_from_0.40.28.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.29] - 2026-04-02 : ダイアログUI統一・IME修正・アップデート修正

### Changed
- **ポップアップダイアログに×（閉じる）ボタンを追加:** AiテーマウィザードDialog、AiテーマカスタマイズDialog、Aiテーマ生成Dialog、Aiお気に入り整理Dialog、テーマウィザードDialog、ZenithDialog、リネームDialogの7つのダイアログに統一的な×ボタンを追加
- **ダイアログのフッターボタン配置を統一:** 全確定型ダイアログのボタンを右寄せに統一し、主アクション（OK/保存）をAccentButtonスタイルに変更。ボタン順序を「キャンセル→OK」（右端が主アクション）に統一

### Fixed
- **アップデート後の再起動ボタンが表示されない不具合を修正:** ダウンロード完了後にバックグラウンドスレッドから UI を更新しようとして失敗していた問題を、XAML バインディングに置き換えて修正
- **テキスト入力時のIME変換候補が消える不具合を修正:** 全テキスト入力フォームのバインディングに `Delay=300` を追加し、IME変換中のUI再描画を抑制。フィードバック・AI入力・検索・ナビペイン等の全入力欄が対象
- **AIフォルダ生成の内部セキュリティ定義漏れを修正:** AiFolderCreate の使用回数上限が未定義だったため追加
