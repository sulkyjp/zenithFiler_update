...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.41.0.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.41.0_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.41.0_delta_from_0.40.31.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.41.0] - 2026-04-04 : 起動高速化・UI硬直解消・実績履歴改善

### Changed
- **レベルバナークリックで実績履歴を表示:** レベルバナー押下時にチャレンジページではなく実績履歴ダイアログを直接表示するよう変更
- **実績履歴ダイアログのドラッグ移動:** ヘッダー部分をドラッグしてウィンドウを自由に移動可能に
- **起動高速化:** テーマ一覧スキャン（50 JSONファイル読み込み）をバックグラウンドで事前キャッシュし、UIスレッドのブロックを約370ms削減。FileLoggerの初期化もWPF起動前にバックグラウンドで実行し、JIT並列化の波及効果も含め合計約1.5秒短縮（2.8秒→1.3秒）
- **パンくずリスト（パスセグメント）の遅延生成:** タブ生成時のShell COM呼び出しをディレクトリ読み込み完了後に遅延実行し、起動時のCOM競合を回避

### Fixed
- **起動直後のUI硬直を解消:** ウェルカムメッセージ表示後にUIがフリーズする問題を修正。CpuIdleService（PerformanceCounter初回読み込み）・TrayIconService（アイコン読み込み）等の重い初期化処理をバックグラウンドスレッドに移動し、UIスレッドのブロックを排除
