...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.2.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.2_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.2_delta_from_0.38.1.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.2] - 2026-03-20 : AI タブ管理 + 統計分析 + お気に入り設定 + タブアクティビティ基盤

### Added
- **AI 稼働統計分析:** 統計ページ上部に AI 分析セクションを追加。操作統計・フォルダ履歴・お気に入り・タブアクティビティ・共起パターン・検索履歴・ワーキングセットの7データソースをクロス分析し、お気に入り未登録の常用フォルダ検出、隠れたワークフロー発見、検索モード最適化など最大7種のインサイトと具体的なアクション提案を表示（Pro 機能、無料版は1日2回まで）
- **タブアクティビティ追跡基盤:** タブの使用パターン（アクセス頻度・滞在時間・共起パターン）を SQLite に記録する TabActivityService を追加。AI タブ管理機能の基盤インフラ
- **フォルダ別表示設定の自動復元:** フォルダごとにソート順・表示モード・フォルダ先頭設定を自動記憶し、再訪問時に自動適用。設定で ON/OFF 切替可能
- **AI タブ自動ロック/クローズ提案:** アクセス頻度・滞在時間・直近性をスコアリングし、タブヘッダーにロック推奨（緑）/クローズ推奨（オレンジ）のインジケーターを表示。「すべて適用」で一括操作可能
- **ワーキングセット自動検出:** A+B ペインの共起パターンからよく使う組み合わせを検出し、ワーキングセットとして保存する提案を表示
- **スマートタブ提案:** 現在のフォルダと関連性の高いフォルダを共起履歴から検出し、新しいタブとして開く提案を表示
- **AI タブ管理ナビペインビュー:** ロック/クローズ提案、ワーキングセット候補、関連フォルダ提案を統合表示するナビペインビューを追加（Pro 機能、無料版は1日2回まで）
- **お気に入り設定ダイアログ:** ナビペインのアイコンバーに歯車アイコンを追加し、お気に入りビューの表示設定をダイアログから変更可能に。フォントファミリ・フォントサイズ・説明文/ツリーライン/アクセス回数バーの表示切替・削除確認・ロックを一元管理。設定はリアルタイムに反映

### Fixed
- **機能使用状況の表示名修正:** ライセンス設定画面の機能使用状況で、AI マニュアル・AI フォルダ分析・AI タブ管理・AI 統計分析の4機能が変数名のまま表示されていた問題を修正。全10言語のローカライズに対応
- **機能使用状況のレイアウト改善:** 機能使用状況グリッドを2列から3列に変更し、よりコンパクトな表示に。不足していた8機能のアイコンも追加

### Changed
- **機能使用状況の基本/AI区分表示:** ライセンス設定画面の機能使用状況を「基本機能」（9個）と「AI 機能」（5個）の2セクションに分割し、見出し付きで表示
- **PRO 機能の無料回数制限を最適化:** 機能特性に応じた3段階の制限に変更。体験重視（テーマ変更: 5回/日）、標準（検索・エクスポート等: 3回/日）、AIコスト高（AI機能: 2回/日）
