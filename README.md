...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.6.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.6_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.6_delta_from_0.40.5.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.6] - 2026-03-27 : カスタムアイコン + ピン件数カスタマイズ + UI改善

### Added
- **お気に入りコンテナにカスタムアイコン:** 仮想フォルダに PackIconLucide ベースのカラーバッジアイコンを設定可能。プリセット64種 + コンテキストメニューから設定。アクセントカラー背景 + 白アイコンのモダンバッジデザイン
- **AIお気に入り整理でアイコンを自動付与:** AI整理時にコンテナの用途に応じたアイコンを自動設定（FolderOpen, Code, Briefcase, Globe 等）
- **お気に入り「よく使う」セクションのピン件数カスタマイズ:** 固定5件から3〜15件をスライダーで選択可能

### Changed
- **お気に入り設定のスライダー化:** フォントサイズ・ピン件数の ComboBox をスライダー + アクセントカラー値表示に統一
- **更新履歴ページの表示高速化:** CHANGELOG を v0.30.0 で分割し、最新分のみ即座に表示。過去の履歴は目次下部の「過去の履歴を表示」ボタンで遅延読み込み

### Fixed
- **デバッグメニューのライセンス一時切替中に機能制限される問題を修正:** CanUseSync/CanUseAsync にデバッグオーバーライド中のバイパスを追加
