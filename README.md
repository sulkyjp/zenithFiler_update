...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.19.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.19_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.19_delta_from_0.40.18.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.19] - 2026-03-30 : オンデマンド検索 + チャレンジ物語 + 初期設定改善

### Added
- **通常検索をオンデマンドファイルシステム検索に変更:** インデックスに依存せず、カレントフォルダのファイルシステムをリアルタイムで走査する方式に変更。高度なクエリ構文をサポート: AND検索（スペース区切り: `report 2024`）、OR検索（`|` 区切り: `doc|pdf`）、除外（`-backup`）、拡張子フィルタ（`ext:pdf`）、ワイルドカード（`*.pdf`）。サイズ/日付フィルタもそのまま適用。再帰検索トグル（FolderTree アイコン）でサブフォルダを含むか選択可能。インデックス検索は従来通りトグルで利用可能
- **チャレンジカードにバックストーリーを追加:** 各実績のツールチップに Tier 1→10 の進化を感じられる10行程度の物語テキストを表示。全14カテゴリ×10 Tier = 140件、10言語対応

### Changed
- **初期設定の最適化:** サイドバー幅280px、ダウンロード日付順ソート有効、グリーティングトーンLv.2、チャレンジ情報含むグリーティング有効、最近追加セクション非表示をデフォルトに変更

### Fixed
- **常に最前面トグルが機能しない問題を修正:** タスクトレイからの復帰処理で Topmost を一時的に true → false に設定していたため、XAML バインディングが上書きされていた。IsAlwaysOnTop が有効な場合は Topmost を維持するよう修正
- **レベルアップ時にバナー演出が発火しない問題を修正:** ShowStartupEffectsEnabled でガードされていたため、スタートアップ演出を無効にするとレベルアップ演出もスキップされていた。レベルアップは常に演出するよう修正
