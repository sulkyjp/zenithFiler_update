...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.1_delta_from_0.38.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.1] - 2026-03-19 : AI フォルダ分析 + バッジ色テーマ対応 + ナビペイン改善

### Added
- **AI / Pro バッジ色テーマ対応:** AI バッジ（紫）と Pro バッジ（オレンジ）の色をテーマごとにカスタマイズ可能に。テーマカスタマイズダイアログの Misc セクションから編集可能。全50テーマにテーマの色調に合わせた個別カラーを設定
- **AI マニュアル構造理解:** アプリの設定項目・キーバインド・テーマ一覧・エフェクト設定などの構造メタデータを AI マニュアルのインデックスに追加。マニュアルに記載のない設定や機能についても AI が回答可能に
- **AI フォルダオーガナイザー:** ナビペインに新しいAIフォルダ整理ビューを追加。フォーカスペインの現在フォルダをAIが分析し、フォルダ作成・ファイル移動・名前変更の整理提案を生成。チェックボックスで選択→一括実行→Undoまで完結する対話的ワークフロー
- **ナビペイン ビュー名ヘッダー:** アイコンバーの下にアクティブビューの名前を表示するヘッダー行を追加。各ビューの下部にあった機能ボタン群をヘッダー右端に移動し、縦スペースを有効活用
- **AI / Pro バッジインジケーター:** ナビペインのアイコンバー・ビューヘッダー・ビュー管理ドロップダウンに AI 機能（紫 Sparkles）とライセンス対象機能（オレンジ Lock）を示すマイクロバッジを追加。ツールチップにも [AI] / [Pro] ラベルを表示。設定でバッジドットの表示/非表示を切替可能
- **設定ページ バッジドット:** Control Deck の左ナビ（AI / Index カテゴリ）、テーマ設定内の AI 診断・AI 生成セクション見出しとボタン、右コンテンツヘッダーにもバッジドット表示を追加。ShowBadgeDots 設定に連動

### Changed
- **AIフォルダ分析 Pro機能化:** AIフォルダ分析を Pro 機能（無料版は1日3回まで）として追加。ナビペインに Pro バッジを表示
- AI関連機能（マニュアルQA・ファイル名提案）の出力言語をアプリの言語設定に同期するよう改善
- **AIフォルダ分析 回答方針改善:** 分析結果を「現状分析→問題点→改善提案」の3セクション構造に変更。改善提案に対するYES/NOボタンで承認後にアクション生成する2段階フローを導入。ユーザーの整理パターンを学習し次回以降の提案精度を向上。ビュー名を「AIフォルダ整理」→「AIフォルダ分析」に変更
- **AIフォルダ分析 UI改善:** チャットエリア内にインライン「分析」ボタンを初期表示し操作を直感的に。チャット履歴のクリアボタンを追加。AIメッセージのテキスト選択・コピーを有効化
- **AIフォルダ分析 説明テキスト・エクスポート・クリア確認:** 承認ボタン下にアクション選択可能な旨の説明テキストを追加。チャット内容をMarkdownファイルとしてエクスポートする機能を追加。クリア時に確認ダイアログを表示するように改善

### Fixed
- プロパティビューのファイル属性表示で Archive フラグが常に表示される問題を修正。Archive を除外し、代わりに圧縮・暗号化属性を表示するように改善
