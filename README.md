...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.3.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.3_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.3_delta_from_0.38.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.3] - 2026-03-20 : AI お気に入り整理 + AI バックアップサマリ + AI 設定改善

### Added
- **AI バックアップサマリ自動生成:** バックアップの変更概要を AI が自然文で自動生成。次回起動時にバックグラウンドで最新バックアップのサマリを高度化。手動バックアップ時も AI サマリを即時生成
- **AI モデル検証機能:** AI 設定のモデル選択区分に「モデル検証」ボタンを追加。指定モデルで実際にリクエストを送信し、使用可否を確認可能に
- **AI 接続エラー詳細表示:** テスト接続・モデル検証の失敗時にプロバイダ別・ステータスコード別の具体的なガイドを表示。全10言語対応
- **AI プロバイダ動作確認注記:** プロバイダ選択欄に「Gemini API でのみ動作確認済み」の注記を追加
- **AIお気に入り整理:** お気に入りツリーをAIが自動で分析・再構成。壊れたリンクの一括削除、関連アイテムのフォルダグループ化、検索用タグの自動付与に対応。AIなしでも壊れたリンクのみ削除可能（Pro機能、無料版は1日2回まで）

### Changed
- **AI 設定 UI レイアウト最適化:** テスト接続ボタンを API キー区分内に統合し、独立していた接続テスト区分を廃止。画面構成を簡素化
- **AI 検証結果のプロバイダ別保持:** テスト接続・モデル検証の結果をプロバイダごとに保持。プロバイダ切替時に他のプロバイダの結果が表示されなくなり、再起動後も結果が復元されるように改善
- **AI エラーメッセージ共通化:** モデル検証のエラーメッセージをプロバイダ別から共通メッセージに統一し、翻訳リソースを簡素化

### Fixed
- **ナビペインヘッダーアイコン右寄せ修正:** バッジアイコン列挿入時にヘッダーアクションボタン群が正しい列に配置されず左寄せになる不具合を修正
