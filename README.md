...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.2.1.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.2.1_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.2.1_delta_from_1.2.0.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.2.1] - 2026-06-11 : Outlook メールのドラッグ＆ドロップ保存を修正

### Fixed
- **Outlook メールのドラッグ＆ドロップ／貼り付け保存を修正:** Outlook からメールをファイル一覧へドロップしても保存されないことがあった。原因はドロップ完了後にデータ取得が走る内部タイミングの問題（複数通で顕著・1通でも環境により失敗）。ドロップ受付中にデータを取り込み切る方式へ修正した。あわせて、パンくず（フォルダ階層）へのメールドロップにも対応し、保存できなかった場合は原因の手がかりを通知とログに必ず残すようにした（従来は無反応だった）
