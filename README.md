...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.41.7.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.41.7_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.41.7_delta_from_0.41.6.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.41.7] - 2026-04-07 : 常駐モード安定化・検索バーUI修正

### Changed
- **トレイアイコンを常時表示:** 常駐モードの ON/OFF に関わらずトレイアイコンを常に表示。トレイメニューから復帰・終了が常に可能に

### Fixed
- **タスクバーダブルクリックによる意図しない終了を修正:** システムコマンド ID 衝突（SC_FORCE_EXIT=0xF120）がダブルクリック時に誤発火していた問題を修正。最小化中の Closing はウィンドウ復帰に変換
- **称号カードのX投稿機能を削除:** X側の制約でテキスト重複が解消できないため機能を削除
- **検索バー枠線色の統一:** 全検索モード（Search/Grep/Index/Filter）でフォーカスなし時の枠線色を BorderBrush に明示統一。Index モードで枠線色が異なって見えていた問題を修正
- **検索バーの文字色統一:** 検索TextBoxにテーマのプライマリテキスト色（TextBrush）を明示指定。ダーク系テーマで入力文字と周囲のラベル色がずれていた問題を修正
- **テーマカードの右クリック反応改善:** テーマカード全域で右クリックするとコンテキストメニューが表示されるよう修正（旧: テキストやアイコン上でのみ反応）
