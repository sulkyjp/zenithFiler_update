...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.6.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.6_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.6_delta_from_0.38.5.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.6] - 2026-03-22 : AIトークン統計 + 起動高速化 + プロパティグリーティング

### Added
- **ナビペイン下段固定機能:** 分割モードの下段ヘッダーにトグルアイコンを追加。有効にすると下段がコンテンツサイズに合わせて自動調整され、上段が残りのスペースを占有。プロパティや最近開いたフォルダなど高さが決まっているビューを下部にピッタリ配置可能。設定はアイテムごとに永続化
- **A/Bペインターミナルタブ:** ファイル一覧と同じタブバーでターミナル（PowerShell）を開けるように。タブバーの SquareTerminal アイコンをクリックして追加。現在のフォルダを作業ディレクトリとして起動し、フォルダタブとターミナルタブをシームレスに切り替え可能。ナビペインのターミナルと設定（フォント・色・テーマ連動）を共有
- **エラー画面からのフィードバック報告:** エラーダイアログおよびAI機能のエラーパネルに「問題を報告」ボタンを追加。クリックするとエラー情報がプリフィルされた状態でフィードバック画面が開き、システム情報・ログ添付もONで即座に報告可能。報告時にエラーダイアログは自動クローズ
- **プロパティビューの起動グリーティング:** ナビペインの「プロパティ」に起動後5秒間だけ時間帯に応じた挨拶メッセージを表示（朝・午前・午後・夕方・夜の6パターン、10言語対応）。フェードアウト後はカレントフォルダのプロパティに自動切り替え
- **プロパティ未選択時のカレントフォルダ表示:** プロパティビューでアイテム未選択時、アクティブペインのカレントフォルダのプロパティ（フォルダ名・パス・更新日時・作成日時・属性）を自動表示。下段固定時のプロパティ領域サイズ変動を解消

### Changed
- **ログレベル体系の刷新:** アプリ全体のログ出力（約302箇所）を正式な Serilog レベル（INF/WRN/ERR）に再分類。従来はすべて INF レベルで出力されていたため、フィードバック送信時にエラーログが収集されなかった問題を解消
- **フィードバックのエラーログ収集強化:** フィードバック送信時に直近5件のエラーログ（従来は1件）を自動収集し、各エラーの直前3件の操作コンテキストも付与。エラー原因の特定精度が大幅に向上
- **AIトークン統計:** 設定画面の統計ページにAIトークン使用量セクションを追加。月別にモデル別トークン数・推定料金、機能別呼び出し回数・トークン合計を表示。主要モデル（Claude/OpenAI/Gemini）の公開単価に基づく料金推定機能付き
- **AIエラーメッセージの種別化:** AIリクエスト失敗時に原因別のメッセージを表示（認証エラー・レート制限・タイムアウト・ネットワーク不通・サーバーエラー等）。従来の汎用メッセージやAPIの生レスポンス表示を廃止し、ユーザーが次のアクションを判断しやすい案内に統一
- **AIリクエスト/レスポンスの構造化ログ:** AI通信の全フェーズ（リクエスト・成功・リトライ・エラー）を `[AI-REQ]`/`[AI-RES]`/`[AI-RETRY]`/`[AI-ERR]` タグ付きで構造化出力。Provider名・モデル名・ステータスコード・処理時間を含み、フィードバック経由でのエラー原因特定が容易に
