...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v1.0.3.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v1.0.3_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v1.0.3_delta_from_1.0.2.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [1.0.3] - 2026-06-07 : 実行時パフォーマンスの改善（チャレンジ表示・ツリー展開の高速化）と起動堅牢性の向上

### Fixed
- **子フォルダが非常に多いフォルダのツリー展開を高速化:** ディレクトリツリーで子フォルダが数千件あるノードを展開する際、重複チェックを総当たり（件数の二乗に比例）で行っていたため展開時に一瞬引っかかることがあった。重複判定を定数時間で行うよう改善し、大量の子フォルダでも滑らかに展開できるようにした
- **起動初期に発生した異常を確実に記録するよう改善:** グローバル例外ハンドラの登録が、設定の読み込みやテーマ適用などの重い初期化処理の後に行われていたため、初期化の前半で問題（テーマデータの破損など）が起きるとログが残らないままサイレントに終了することがあった。ハンドラ登録を起動のごく冒頭へ前倒しし、初期化中の異常も確実に捕捉・記録できるようにした
- **マスコット演出のオン/オフ切替を繰り返した際のメモリ累積を修正:** ステータスバーのマスコット演出を有効/無効に切り替えるたびに、内部のイベント購読が解除されず少しずつメモリを消費していく不具合を修正した（既定オフのため、機能を有効にしている場合のみ影響）
- **チャレンジ／スティックマン報酬カテゴリの表示を高速化:** 設定画面でチャレンジ（またはスティックマン報酬）のカテゴリを開くたびに、数百枚の達成カードを毎回ゼロから作り直していたため、開くたびに数秒のひっかかりが発生していた。進捗や表示言語が変化したときだけ作り直すようにし、変化がない再表示は瞬時に切り替わるようにした（進捗の鮮度は維持。再訪コストを約3秒→約30msに短縮）
