...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.24.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.24_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.24_delta_from_0.40.23.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.24] - 2026-03-31 : 検索バー2段目・マルチディスプレイ修正・バグ修正

### Added
- **検索バー2段目表示オプション:** 2画面（左右モード）時に検索バーをツールバーの2段目に自動移動するオプションを表示設定に追加。検索バーの入力領域を広く確保可能
- **フィードバックボタン:** タイトルバーのアイコン群にフィードバックボタンを追加。クリックでブラウザからフィードバックページを直接開ける

### Changed
- **通常検索の配下検索デフォルト化:** 通常検索で常にサブフォルダを含めて検索するよう変更し、分かりづらかった再帰検索トグルボタンを削除
- **二つ名バックストーリー生成の強化:** AI 生成時、各カテゴリの最高到達実績のバックストーリーを参考情報としてプロンプトに追加。より世界観に沿った統合バックストーリーを生成
- **AI 不使用時の機械合成バックストーリー:** AI が使えない場合、各カテゴリの最高 Tier 実績名と達成条件を列挙したフォールバックバックストーリーを自動生成
- **デバッグ再生成の機械合成対応:** AI OFF 時にデバッグページの二つ名・バックストーリー再生成が機械合成にフォールバックするように変更

### Fixed
- **ナビペイン非表示時の空白表示を修正:** 右ナビペインまたは左ナビペインを非表示にした状態で終了すると、次回起動時にペイン配置部分が空白になる問題を修正。左スプリッター列の動的制御と ContentRendered 時のレイアウト再適用を追加
- **外部ディスプレイ切断時のウィンドウ画面外問題を修正:** 拡張ディスプレイを取り外した際、ウィンドウが画面外に残り操作不能になる問題を修正。起動時および実行中のディスプレイ構成変更を検知し、自動的にプライマリモニタへ最大化で移送
- **検索結果のパスリンクをフォルダリンクに修正:** 検索結果一覧のパス列クリック時、ファイルではなく格納フォルダを開くように変更
- **AI トグルの同期修正:** ステータスバーの AI トグルと設定画面の AI チェックボックスが双方向で同期するように修正
- **AI トグル状態が反映されない問題の修正:** AI ON/OFF 切替後、二つ名・バックストーリー生成に状態が反映されず常に逆の動作になる不具合を修正
- **バックストーリー AI 生成の重複リクエスト修正:** 生成中に他のチャレンジアクションが発火すると同一バックストーリーの AI リクエストが二重に走る問題を修正
- **タブ管理ビューの×ボタンでタブが閉じない問題を修正:** ×ボタンクリック時にドラッグ操作が誤って開始され、Click イベントが発火しない不具合を修正
