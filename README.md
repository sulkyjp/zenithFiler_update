...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.38.5.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.38.5_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.38.5_delta_from_0.38.4.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.38.5] - 2026-03-20 : テキスト比較メニュー文言改善

### Changed
- **テキスト比較メニュー文言改善:** 比較メニューにペイン情報を追加。同ペイン比較は「テキスト比較（Aペイン内）」、クロスペイン比較は「テキスト比較（Aペイン ↔ Bペイン）」のように、どのペインのファイルを比較しているかが一目でわかるように改善
