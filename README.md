...
| ファイル | 内容 |
|---|---|
| `ZenithFiler_v0.40.27.zip` | **完全版** — .NET ランタイム同梱。初回導入や環境移行に |
| `ZenithFiler_v0.40.27_patch.zip` | **軽量版** — ランタイム除外。既存環境のアップデートに |
| `ZenithFiler_v0.40.27_delta_from_0.40.26.zip` | **差分版** — 前バージョンから変更されたファイルのみ |
<!-- download-table:end -->

> 過去のバージョンは [Releases](https://github.com/sulkyjp/zenithFiler_update/releases) ページから取得できます。

<!-- latest-changes:begin -->
## Latest Changes — [0.40.27] - 2026-04-02 : AIフォルダ生成・Ollama改善・推薦Tips改善

### Added
- **AI フォルダ生成機能:** AIにプロンプトで用途を説明し、最適なフォルダ構成を提案・生成する機能を追加。新規作成に加え、既存フォルダとの統合（リネーム提案）にも対応。4軸の自己評価（適合度・構造・命名・拡張性）をプレビュー表示。重視ポイントのチップボタンでAIの提案方針を調整可能。追加指示による再生成、既存ファイルのパターン分析（年月・連番等）による網羅的提案に対応
- **スクリーンショット自動撮影モード:** `--capture-screenshots` 引数でアプリを起動すると、定義済みシナリオ（メイン画面・ナビペイン・Control Deck 等）に従って各画面を自動撮影し `docs/screenshots/` に PNG 保存。マニュアル画像の自動更新に使用

### Changed
- **機能推薦Tips の表示形式改善:** 推薦対象の機能名をヘッダーとして明示表示（例: `💡 [テキスト比較]`）し、何の機能推薦かを一目でわかるように改善
- **Ollama セットアップガイド:** Ollama タブを3ステップのガイド形式に再構成（① インストール → ② サーバ起動 → ③ モデルDL）。各ステップの状態に応じて表示を切替え、手順が明確にわかるように改善
- **AI プロバイダ切替時のモデル連動:** プロバイダを切り替えた際にモデルリスト・APIキー表示が前のプロバイダの情報で残る問題を修正
- **コンテキストメニュー暗黙スタイル化:** `ItemContainerStyle` を個別指定から暗黙の MenuItem スタイルに移行。Separator との型衝突によるクラッシュを根本解決

### Fixed
- **Ollama 推奨モデル取得時のフリーズ修正:** 進捗報告を100ms間隔にスロットリングし、UIスレッドの飽和を防止
- **AI フォルダ生成のトークン上限対応:** MaxTokens を32768に引き上げ、途中切断時の警告表示とエラーログ出力を追加
- **フィーチャー表示名のローカライズ漏れ修正:** AI トークン統計画面で一部機能名が英語キーのまま表示されていた問題を修正
- **マニュアル画像表示対応:** ドキュメントビューアで Markdown 内の画像（`![alt](path)`）を表示可能に。相対パスの自動解決、角丸・枠線装飾、MaxWidth 制約を適用
- **機能推薦Tips が一瞬で消える問題を修正:** フェードアウトアニメーション後に Opacity が 0 で保持され、再表示時に見えなくなっていた問題を修正
- **機能推薦Tips 表示中のプロパティ重複表示を修正:** Tips 表示中はファイルプロパティ（パス・サイズ・日時等）を非表示にし、表示の重複を解消
